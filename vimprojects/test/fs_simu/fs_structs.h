...
#define DMAP_MAXSIZE DIR_MAXNUM/64	//目录表位图的长度
...
#define NICFREE BNODE_MAXNUM     	//空闲物理块栈的大小
#define NICINOD INODE_MAXNUM     	//空闲i节点数组大小

#define IMAP_MAXSIZE INODE_MAXNUM/64 		//i节点位图的长度
#define BMAP_MAXSIZE BNODE_MAXNUM/64  		//物理块位图的长度
...
struct imap
{
	unsigned long long bits[IMAP_MAXSIZE];
};
...
struct bmap
{
	unsigned long long bits[BMAP_MAXSIZE];
};
...
struct dir_info
{
	 ...
	 */
	int index;                                  //目录表中可用的位置。
 	unsigned long long dmap[DMAP_MAXSIZE];	//位图
};
