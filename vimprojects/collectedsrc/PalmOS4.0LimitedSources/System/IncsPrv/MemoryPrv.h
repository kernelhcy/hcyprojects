 ...
 *		05/11/00	kwk	Added MemGetRomNVParams declaration.
 *		07/23/00	kwk	Added initCodeOffset3 to StorageHeaderType.
 *		07/25/00	kwk	Moved initCodeOffset3 to be _after_ the reserved space.
 *		08/19/00	kja	Added version 5 items (company & HAL ID) to CardHeaderType.
 *		08/24/00	SCL	Added halCodeOffset2 to CardHeaderType.
 *
 *****************************************************************************/
...
//  a byte
#if (CPU_TYPE == CPU_68K)	// !!! Not really a good test; bitfield layout is compiler-specific, not CPU-specific
  #define memChunkFlags(p) 		(*((UInt8 *)p) & 0xF0)
#else
  #define memChunkFlags(p)		\
			  (((MemChunkHeaderPtr)p)->free << 7)		\
			| (((MemChunkHeaderPtr)p)->moved << 6)		\
			| (((MemChunkHeaderPtr)p)->unused2 << 5)	\
			| (((MemChunkHeaderPtr)p)->unused3 << 4)
#endif
...
typedef struct CardHeaderType {
	...
	UInt32		halCodeOffset;				// v4: offset from CardHeader to HAL code
											...
											//     All v5 ROMs must contain non-zero company and HAL IDs.
	UInt32		companyID;					// v5: company code of device required by ROM (e.g. 'palm')
											//     this should be the same as hwrOEMCompanyID
	UInt32		halID;						// v5: HAL ID code of device required by ROM (e.g. 'sumo')
											//     this should be the same as hwrOEMHALID
	UInt32		romVersion;					// v5: ROM numeric version, in bit-sliced format (same as other
											//     version fields). This field is not used by the OS.
	UInt8		romVersionString[32];		// v5: ROM string version, in ASCIIZ format. This field is not
											//     used by the OS, except for being displayed by the debug stub.
	UInt32		halCodeOffset2;				// v5: offset from CardHeader to second HAL code resource

	UInt8		reserved[82];				// to bring us to 0x100 alignment
	} CardHeaderType;
...
typedef struct {
	...
	// Since nvParams could potentially grow as well, new fields should
	// be added _after_ the reserved area, so that it can get consumed
	// from both ends without causing backwards compatibility issues.
	
	UInt32		initCodeOffset3;			// third init code for store, if any
	
	// CRC value - this value MUST be at the very end of
	// the structure.
...
void				MemGetRomNVParams(SysNVParamsType* paramsP)
							SYS_TRAP(sysTrapMemGetRomNVParams);
