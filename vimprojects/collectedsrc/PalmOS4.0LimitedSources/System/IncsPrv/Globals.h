 ...
 *		06/09/00 vsm	Added app sub-font info pointer.
 *		09/26/00	kwk	Added inlineClauseOffset, and replaced inlineHighliteLen
 *							with inlineClauseLen.
 *		10/02/00	kwk	Moved inlineClauseLen (was inlineHighliteLen) back to
 *							exactly its previous location.
 *		01/16/01	jhl	Added BtStackLib globals pointer (usurped sysReserved40DWord1)
 *
 *****************************************************************************/
...
typedef struct FixedGlobalsType {
	...
	Boolean					inlineDefTerm;							// True if deferred termination
	...
	UInt16					inlineClauseLen;						// Length of clause text in inline area.
	...
	// <3/14/00 SCL> ExpansionMgr and FileSystemMgr globals (Palm OS 3.5 or later)
	// <8/17/00 JED> Swapped with sysReserved31DWord1&2, renamed them to 3&4 to match actual usage.
	UInt32					expGlobalsP;				// was sysReserved31DWord3
	UInt32					vfsGlobalsP;				// was sysReserved31DWord4 
	...
	//-----------------------------------------------------------------------
	// New Globals added for the 4.0 release...
	//-----------------------------------------------------------------------

	// bob 3/13/00 pointer to queue of notifications from interrupt handlers.
	void *					sysNotifyInterruptGlobalsP;
	
	// vivek 06/09/00 pointer to application defined sub-fonts (double byte fonts).
	void *					sysAppSubFontListPtr;


	// ADH 4/30/00 Global configuration index for USB hardware
	UInt16					sysUSBConfiguration;		
	
	// kwk 05/10/00 pointer to dispatch table used by IntlDispatch trap.
	void *					intlDispatchTableP;

	//WK 6/2/00 Globals auto locking device.
	Boolean					sysAutoLockUponPowerOff;  	//auto lock upon device power off
	UInt32					sysHardPowerEvtTicks;			// Ticks of last auto off idle timer event - used to support auto lock
	UInt32					sysAutoLockTimeoutSecs;		//auto lock timeout in seconds
	
	// peter 06/21/00 Pointer to attention manager's globals.
	void *					attnGlobalsP;

	// lrt	07/28/00 Pointer to shared Expansion Manager & VFS Manager globals
	void *					expVfsSharedGlobalsP;

	// TKe 08/22/00	Bluetooth Exchange Library globals pointer
	void *					btExgLibGlobalsP;

	// bob 08/28/00	Test Harness globals
	void *					testHarnessGlobalsP;				
	// srj 09/19/00	Power Manager Globsls
	void *					pwrManagerGlobalsP;

	// kwk 09/27/00	Clause offset for FEP.
	UInt16					inlineClauseOffset;					// Offset from start of inline to clause.

	// djk 10/17/00 	Bluetooth Library globals pointer
	void *					btLibGlobalsP;

	// WK	 11/2/00    password timeout globals.
	Boolean 				sysSetPwdTimeoutAllowed;
	UInt32					sysSetPwdTimeoutSecs;
	UInt32					sysSetPwdTimeoutTicks;

	//WK   	 11/5/00    Big and small ROMs' lock global.    
	UInt16					dbgLockout;
	
	// acs  1/3/01		gamma table stored as a ROM token
	void *					hwrGammaTableP;
	
	// <01/16/01 jhl> Necessarily distinct from btLibGlobalsP.
	void *					btStackLibGlobalsP;				// was sysReserved40DWord1

	// Some globals reserved for future use (only available in 4.0 or later)
	// please keep these at the end of the '4.0' section (DOLATER: remove this comment line)
	UInt32					sysReserved40DWord2;
	UInt32					sysReserved40DWord3;
	UInt32					sysReserved40DWord4;
...
#define	GInlineClauseOffset		(((LowMemHdrType*)PilotGlobalsP)->globals.inlineClauseOffset)
#define	GInlineClauseLen			(((LowMemHdrType*)PilotGlobalsP)->globals.inlineClauseLen)
...
#define	GSysExpMgrGlobalsP	(((LowMemHdrType*)PilotGlobalsP)->globals.expGlobalsP)
#define	GVFSGlobalsP			(((LowMemHdrType*)PilotGlobalsP)->globals.vfsGlobalsP)

#define	GSysNotifyInterruptGlobalsP (((LowMemHdrType*)PilotGlobalsP)->globals.sysNotifyInterruptGlobalsP)

#define	GIntlDispatchTableP	(((LowMemHdrType*)PilotGlobalsP)->globals.intlDispatchTableP)

#define 	GUSBConfiguration		(((LowMemHdrType*)PilotGlobalsP)->globals.sysUSBConfiguration)

#define 	GAppSubFontListPtr	(((LowMemHdrType*)PilotGlobalsP)->globals.sysAppSubFontListPtr)

#define GSysAutoLockUponPowerOff (((LowMemHdrType*)PilotGlobalsP)->globals.sysAutoLockUponPowerOff)
#define GSysHardPowerEvtTicks		(((LowMemHdrType*)PilotGlobalsP)->globals.sysHardPowerEvtTicks)
#define GSysAutoLockTimeoutSecs (((LowMemHdrType*)PilotGlobalsP)->globals.sysAutoLockTimeoutSecs)

#define	GAttnIndicatorIsAllowed (((LowMemHdrType*)PilotGlobalsP)->globals.attnIndicatorIsAllowed)
#define	GAttnIndicatorIsEnabled (((LowMemHdrType*)PilotGlobalsP)->globals.attnIndicatorIsEnabled)
#define	GAttnIndicatorFrame	(((LowMemHdrType*)PilotGlobalsP)->globals.attnIndicatorFrame)
#define	GAttnIndicatorBlinkPattern	(((LowMemHdrType*)PilotGlobalsP)->globals.attnIndicatorBlinkPattern)
#define	GAttnIndicatorBitsBehind	(((LowMemHdrType*)PilotGlobalsP)->globals.attnIndicatorBitsBehind)

#define	GAttnIndicatorLastTick	(((LowMemHdrType*)PilotGlobalsP)->globals.attnIndicatorLastTick)
#define	GAttnGlobalsP				(((LowMemHdrType*)PilotGlobalsP)->globals.attnGlobalsP)

#define GExpVfsSharedGlobalsP	(((LowMemHdrType*)PilotGlobalsP)->globals.expVfsSharedGlobalsP)

#define	GBtExgLibGlobalsP		(((LowMemHdrType*)PilotGlobalsP)->globals.btExgLibGlobalsP)

#define	GTestHarnessGlobalsP	(((LowMemHdrType*)PilotGlobalsP)->globals.testHarnessGlobalsP)

#define	GPwrManagerGlobalsP	(((LowMemHdrType*)PilotGlobalsP)->globals.pwrManagerGlobalsP)

#define	GBtLibGlobalsP		(((LowMemHdrType*)PilotGlobalsP)->globals.btLibGlobalsP)

#define GSysSetPwdTimeoutSecs (((LowMemHdrType*)PilotGlobalsP)->globals.sysSetPwdTimeoutSecs)
#define GSysSetPwdTimeoutTicks (((LowMemHdrType*)PilotGlobalsP)->globals.sysSetPwdTimeoutTicks)

#define GDbgLockout (((LowMemHdrType*)PilotGlobalsP)->globals.dbgLockout)

#define	GBtStackLibGlobalsP		(((LowMemHdrType*)PilotGlobalsP)->globals.btStackLibGlobalsP)
