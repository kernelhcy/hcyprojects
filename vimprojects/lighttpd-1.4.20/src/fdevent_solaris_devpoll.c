...
static int
fdevent_solaris_devpoll_event_add(fdevents * ev, int fde_ndx, int fd,
								  int events)
{
	struct pollfd pfd;
	int add = 0;
...
{
	/*
	 * a forked process does only inherit the filedescriptor, but every
	 * operation on the device will lead to a EACCES 
	 */
	if ((ev->devpoll_fd = open("/dev/poll", O_RDWR)) < 0)
	{
		fprintf(stderr,
				"%s.%d: opening /dev/poll failed (%s), try to set server.event-handler = \"poll\" or \"select\"\n",
...
int fdevent_solaris_devpoll_init(fdevents * ev)
{
	...
	fprintf(stderr,
			"%s.%d: solaris-devpoll not supported, try to set server.event-handler = \"poll\" or \"select\"\n",
			__FILE__, __LINE__);
