...
SETDEFAULTS_FUNC(mod_userdir_set_defaults)
{
	...
	config_values_t cv[] = {
		...
		,						/* 0 */
		{"userdir.exclude-user", NULL, T_CONFIG_ARRAY,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 1 */
		{"userdir.include-user", NULL, T_CONFIG_ARRAY,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 2 */
		{"userdir.basepath", NULL, T_CONFIG_STRING,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 3 */
		{"userdir.letterhomes", NULL, T_CONFIG_BOOLEAN,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 4 */
...
#define PATCH(x) \
	p->conf.x = s->x;
static int
mod_userdir_patch_connection(server * srv, connection * con, plugin_data * p)
{
	size_t i, j;
		...
		for (j = 0; j < dc->value->used; j++)
		{
			...
			if (buffer_is_equal_string(du->key, CONST_STR_LEN("userdir.path")))
			{
				PATCH(path);
					   ...
					   (du->key, CONST_STR_LEN("userdir.exclude-user")))
			{
				PATCH(exclude_user);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("userdir.include-user")))
			{
				PATCH(include_user);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("userdir.basepath")))
			{
				PATCH(basepath);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("userdir.letterhomes")))
			{
				PATCH(letterhomes);
			}
		}
...
URIHANDLER_FUNC(mod_userdir_docroot_handler)
{
	...
	if (con->uri.path->ptr[0] != '/' || con->uri.path->ptr[1] != '~')
		return HANDLER_GO_ON;
	...
	buffer_copy_string_len(p->username, con->uri.path->ptr + 2,
						   rel_url - (con->uri.path->ptr + 2));
	...
	} else
	{
		char *cp;
		/*
		 * check if the username is valid a request for /~../ should lead to a
		 * directory traversal limiting to [-_a-z0-9.] should fix it 
		 */
	...
	/*
	 * the physical rel_path is basically the same as uri.path; but it is
	 * converted to lowercase in case of force_lowercase_filenames and some
	 * special handling for trailing '.', ' ' and '/' on windows we assume that 
	 * no docroot/physical handler changed this (docroot should only set the
	 * docroot/server name, phyiscal should only change the phyiscal.path; the
	 * exception mod_secure_download doesn't work with userdir anyway) 
	 */
	BUFFER_APPEND_SLASH(p->temp_path);
	/*
	 ...
	 * TODO: what to do with trailing dots in usernames on windows? they may
	 * result in the same directory as a username without them. 
	 */
