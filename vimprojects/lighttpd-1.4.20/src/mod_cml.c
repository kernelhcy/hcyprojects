...
SETDEFAULTS_FUNC(mod_cml_set_defaults)
{
	...
	config_values_t cv[] = {
		{"cml.extension", NULL, T_CONFIG_STRING, T_CONFIG_SCOPE_CONNECTION}
		,						/* 0 */
		{"cml.memcache-hosts", NULL, T_CONFIG_ARRAY,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 1 */
		{"cml.memcache-namespace", NULL, T_CONFIG_STRING,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 2 */
		{"cml.power-magnet", NULL, T_CONFIG_STRING,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 3 */
	...
	p->config_storage =
		malloc(srv->config_context->used * sizeof(specific_config *));
...
#define PATCH(x) \
	p->conf.x = s->x;
static int
mod_cml_patch_connection(server * srv, connection * con, plugin_data * p)
{
	size_t i, j;
		...
		for (j = 0; j < dc->value->used; j++)
		{
			...
			if (buffer_is_equal_string(du->key, CONST_STR_LEN("cml.extension")))
			{
				PATCH(ext);
					   ...
					   (du->key, CONST_STR_LEN("cml.memcache-hosts")))
			{
#if defined(HAVE_MEMCACHE_H)
				PATCH(mc);
#endif
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("cml.memcache-namespace")))
			{
				PATCH(mc_namespace);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("cml.power-magnet")))
			{
				PATCH(power_magnet);
			}
		}
...
int
cache_call_lua(server * srv, connection * con, plugin_data * p,
			   buffer * cml_file)
{
	buffer *b;
	char *c;
	...
	/*
	 * prepare variables - cookie-based - get-param-based 
	 */
...
URIHANDLER_FUNC(mod_cml_power_magnet)
{
	...
	if (buffer_is_empty(p->conf.power_magnet))
		return HANDLER_GO_ON;
...
URIHANDLER_FUNC(mod_cml_is_handled)
{
	...
	if (buffer_is_empty(p->conf.ext))
		return HANDLER_GO_ON;

	if (!buffer_is_equal_right_len
		(con->physical.path, p->conf.ext, p->conf.ext->used - 1))
	{
		return HANDLER_GO_ON;
	}
