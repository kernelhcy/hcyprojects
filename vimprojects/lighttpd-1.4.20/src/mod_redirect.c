...
SETDEFAULTS_FUNC(mod_redirect_set_defaults)
{
	...
	config_values_t cv[] = {
		{"url.redirect", NULL, T_CONFIG_LOCAL, T_CONFIG_SCOPE_CONNECTION}
		,						/* 0 */
	...
	for (i = 0; i < srv->config_context->used; i++)
	{
		...
		if (du->type != TYPE_ARRAY)
		{
			log_error_write(srv, __FILE__, __LINE__, "sss",
							"unexpected type for key: ", "url.redirect",
							"array of strings");
...
#ifdef HAVE_PCRE_H
static int
mod_redirect_patch_connection(server * srv, connection * con, plugin_data * p)
{
	size_t i, j;
...
#endif
static handler_t
mod_redirect_uri_handler(server * srv, connection * con, void *p_data)
{
#ifdef HAVE_PCRE_H
...
# define N 10
		...
		if ((n =
			 pcre_exec(match, extra, p->match_buf->ptr,
		...
		} else
		{
			...
			for (k = 0; k < pattern_len; k++)
			{
				if (pattern[k] == '$' || pattern[k] == '%')
				{
					...
					buffer_append_string_len(p->location,
			...
			buffer_append_string_len(p->location, pattern + start,
									 pattern_len - start);

			pcre_free(list);

			response_header_insert(srv, con, CONST_STR_LEN("Location"),
								   CONST_BUF_LEN(p->location));
