...
#if defined(HAVE_PCRE_H)
			if (s->trigger_regex)
				pcre_free(s->trigger_regex);
			if (s->download_regex)
				pcre_free(s->download_regex);
#endif
#if defined(HAVE_GDBM_H)
			if (s->db)
				gdbm_close(s->db);
...
SETDEFAULTS_FUNC(mod_trigger_b4_dl_set_defaults)
{
	...
	config_values_t cv[] = {
		{"trigger-before-download.gdbm-filename", NULL, T_CONFIG_STRING,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 0 */
		{"trigger-before-download.trigger-url", NULL, T_CONFIG_STRING,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 1 */
		{"trigger-before-download.download-url", NULL, T_CONFIG_STRING,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 2 */
		{"trigger-before-download.deny-url", NULL, T_CONFIG_STRING,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 3 */
		{"trigger-before-download.trigger-timeout", NULL, T_CONFIG_SHORT,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 4 */
		{"trigger-before-download.memcache-hosts", NULL, T_CONFIG_ARRAY,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 5 */
		{"trigger-before-download.memcache-namespace", NULL,
		 T_CONFIG_STRING,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 6 */
		{"trigger-before-download.debug", NULL, T_CONFIG_BOOLEAN,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 7 */
	...
	for (i = 0; i < srv->config_context->used; i++)
	{
		...
		if (!buffer_is_empty(s->db_filename))
		{
			if (NULL ==
				(s->db =
				 gdbm_open(s->db_filename->ptr, 4096,
						   GDBM_WRCREAT | GDBM_NOLOCK, S_IRUSR | S_IWUSR, 0)))
			{
		...
		if (!buffer_is_empty(s->download_url))
		{
			if (NULL ==
				(s->download_regex =
				 pcre_compile(s->download_url->ptr, 0, &errptr, &erroff, NULL)))
			{
		...
		if (!buffer_is_empty(s->trigger_url))
		{
			if (NULL ==
				(s->trigger_regex =
				 pcre_compile(s->trigger_url->ptr, 0, &errptr, &erroff, NULL)))
			{
			...
			for (k = 0; k < s->mc_hosts->used; k++)
			{
				...
				if (0 != mc_server_add4(s->mc, ds->value->ptr))
				{
					log_error_write(srv, __FILE__, __LINE__, "sb",
									"connection to host failed:", ds->value);
...
#define PATCH(x) \
	p->conf.x = s->x;
static int
mod_trigger_b4_dl_patch_connection(server * srv, connection * con,
								   plugin_data * p)
{
	size_t i, j;
...
# define N 10
	...
	if (!p->conf.trigger_regex || !p->conf.download_regex)
		return HANDLER_GO_ON;
...
# endif
	 ...
	 */
	if ((n =
		 pcre_exec(p->conf.trigger_regex, NULL, con->uri.path->ptr,
				   con->uri.path->used - 1, 0, 0, ovec, 3 * N)) < 0)
	{
...
# if defined(HAVE_GDBM_H)
		if (p->conf.db)
		{
			...
			{
				log_error_write(srv, __FILE__, __LINE__, "s", "insert failed");
			}
		}
# endif
# if defined(HAVE_MEMCACHE_H)
		if (p->conf.mc)
		{
			...
			{
				log_error_write(srv, __FILE__, __LINE__, "sb",
								"(debug) triggered IP:", p->tmp_buf);
			}

			if (0 != mc_set(p->conf.mc,
							CONST_BUF_LEN(p->tmp_buf),
							(char *) &(srv->cur_ts),
							sizeof(srv->cur_ts), p->conf.trigger_timeout, 0))
			{
				log_error_write(srv, __FILE__, __LINE__, "s", "insert failed");
			}
		}
# endif
	 ...
	 */
	if ((n =
		 pcre_exec(p->conf.download_regex, NULL, con->uri.path->ptr,
				   con->uri.path->used - 1, 0, 0, ovec, 3 * N)) < 0)
	{
		if (n != PCRE_ERROR_NOMATCH)
		{
...
# if defined(HAVE_MEMCACHE_H)
		if (p->conf.mc)
		{
			 ...
			 *
			 */
			if (NULL == (r = mc_aget(p->conf.mc, CONST_BUF_LEN(p->tmp_buf))))
			{
...
TRIGGER_FUNC(mod_trigger_b4_dl_handle_trigger)
{
	...
	for (i = 0; i < srv->config_context->used; i++)
	{
		...
		/*
		 * according to the manual this loop + delete does delete all entries
		 * on its way we don't care as the next round will remove them. We
		 * don't have to perfect here. 
		 */
		for (key = gdbm_firstkey(s->db); key.dptr;
			 key = gdbm_nextkey(s->db, okey))
		{
			time_t last_hit;
			if (okey.dptr)
			{
		 ...
		 * reorg once a day 
		 */
		if ((srv->cur_ts % (60 * 60 * 24) != 0))
			gdbm_reorganize(s->db);
	}
	return HANDLER_GO_ON;
}
#endif
