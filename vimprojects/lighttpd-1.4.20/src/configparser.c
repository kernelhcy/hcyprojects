/*
 * Driver template for the LEMON parser generator. ** The author disclaims
 * copyright to this source code. 
 */
/*
 * First off, code is include which follows the "include" declaration ** in the 
 * input file. 
 */
#include <stdio.h>
...
data_unset *configparser_merge_data(data_unset * op1, const data_unset * op2)
{
	...
	if (op1->type != op2->type)
	{
		...
		} else if (op1->type == TYPE_INTEGER && op2->type == TYPE_STRING)
		{
			...
			buffer_append_string_buffer(ds->value,
...
/*
 * Next is all token values, in a form suitable for use by makeheaders. ** This 
 * section will be null unless lemon is run with the -m switch. 
 */
/*
...
#endif
/*
 * The next thing included is series of defines which control ** various
 * aspects of the generated parser. ** YYCODETYPE is the data type used for
 * storing terminal ** and nonterminal numbers.  "unsigned char" is ** used if
 * there are fewer than 250 terminals ** and nonterminals.  "int" is used
 * otherwise. ** YYNOCODE is a number of type YYCODETYPE which corresponds **
 * to no legal terminal or nonterminal number.  This ** number is used to fill
 * in empty slots of the hash ** table. ** YYFALLBACK If defined, this
 * indicates that one or more tokens ** have fall-back values which should be
 * used if the ** original value of the token will not parse. ** YYACTIONTYPE
 * is the data type used for storing terminal ** and nonterminal numbers.
 * "unsigned char" is ** used if there are fewer than 250 rules and ** states
 * combined.  "int" is used otherwise. ** configparserTOKENTYPE is the data
 * type used for minor tokens given ** directly to the parser from the
 * tokenizer. ** YYMINORTYPE is the data type used for all minor tokens. **
 * This is typically a union of many types, one of ** which is
 * configparserTOKENTYPE.  The entry in the union ** for base tokens is called
 * "yy0". ** YYSTACKDEPTH is the maximum depth of the parser's stack. **
 ...
 * YYNSTATE the combined number of states. ** YYNRULE the number of rules in
 * the grammar ** YYERRORSYMBOL is the code number of the error symbol.  If not
 ...
 * action integer. ** ** Suppose the action integer is N.  Then the action is
 * determined as ** follows ** ** 0 <= N < YYNSTATE Shift N.  That is, push the 
 * lookahead ** token onto the stack and goto state N. ** ** YYNSTATE <= N <
 * YYNSTATE+YYNRULE Reduce by rule N-YYNSTATE. ** ** N == YYNSTATE+YYNRULE A
 * syntax error has occurred. ** ** N == YYNSTATE+YYNRULE+1 The parser accepts
 * its input. ** ** N == YYNSTATE+YYNRULE+2 No such action.  Denotes unused **
 * slots in the yy_action[] table. ** ** The action table is constructed as a
 * single large table named yy_action[]. ** Given state S and lookahead X, the
 * action is computed as ** ** yy_action[ yy_shift_ofst[S] + X ] ** ** If the
 * index value yy_shift_ofst[S]+X is out of range or if the value **
 * yy_lookahead[yy_shift_ofst[S]+X] is not equal to X or if yy_shift_ofst[S] ** 
 * is equal to YY_SHIFT_USE_DFLT, it means that the action is not in the table
 * ** and that yy_default[S] should be used instead. ** ** The formula above is 
 * for computing the action when the lookahead is ** a terminal symbol.  If the 
 * lookahead is a non-terminal (as occurs after ** a reduce action) then the
 * yy_reduce_ofst[] array is used in place of ** the yy_shift_ofst[] array and
 * YY_REDUCE_USE_DFLT is used in place of ** YY_SHIFT_USE_DFLT. ** ** The
 * following are the tables generated in this section: ** ** yy_action[] A
 * single table containing all actions. ** yy_lookahead[] A table containing
 * the lookahead for each entry in ** yy_action.  Used to detect hash
 * collisions. ** yy_shift_ofst[] For each state, the offset into yy_action for
 * ** shifting terminals. ** yy_reduce_ofst[] For each state, the offset into
 * yy_action for ** shifting non-terminals after a reduce. ** yy_default[]
 * Default action for each state. 
 */
static YYACTIONTYPE yy_action[] = {
	/*
	 * 0 
	 */ 2, 3, 4, 5, 13, 14, 63, 15, 7, 45,
	/*
	 * 10 
	 */ 20, 88, 16, 46, 28, 49, 41, 10, 40, 25,
	/*
	 * 20 
	 */ 22, 50, 46, 8, 15, 104, 1, 20, 28, 18,
	/*
	 * 30 
	 */ 58, 60, 6, 25, 22, 40, 47, 62, 11, 46,
	/*
	 * 40 
	 */ 20, 9, 23, 24, 26, 29, 89, 58, 60, 10,
	/*
	 * 50 
	 */ 17, 38, 28, 27, 37, 19, 30, 25, 22, 34,
	/*
	 * 60 
	 */ 15, 100, 20, 20, 23, 24, 26, 12, 19, 31,
	/*
	 * 70 
	 */ 32, 40, 19, 44, 43, 46, 95, 35, 90, 89,
	/*
	 * 80 
	 */ 28, 49, 42, 58, 60, 25, 22, 59, 28, 27,
	/*
	 * 90 
	 */ 33, 48, 52, 25, 22, 34, 28, 49, 51, 28,
	/*
	 * 100 
	 */ 36, 25, 22, 61, 25, 22, 89, 28, 39, 89,
	/*
	 * 110 
	 */ 89, 89, 25, 22, 54, 55, 56, 57, 89, 28,
	/*
	 * 120 
	 */ 53, 21, 89, 89, 25, 22, 25, 22,
};
static YYCODETYPE yy_lookahead[] = {
	/*
	 * 0 
	 */ 29, 30, 31, 32, 33, 34, 0, 1, 44, 38,
	/*
	 * 10 
	 */ 4, 15, 41, 16, 35, 36, 45, 46, 12, 40,
	/*
	 * 20 
	 */ 41, 42, 16, 15, 1, 27, 28, 4, 35, 36,
	/*
	 * 30 
	 */ 24, 25, 1, 40, 41, 12, 17, 14, 13, 16,
	/*
	 * 40 
	 */ 4, 38, 6, 7, 8, 9, 15, 24, 25, 46,
	/*
	 * 50 
	 */ 2, 3, 35, 36, 37, 5, 39, 40, 41, 42,
	/*
	 * 60 
	 */ 1, 11, 4, 4, 6, 7, 8, 28, 5, 9,
	/*
	 * 70 
	 */ 10, 12, 5, 14, 28, 16, 13, 11, 13, 47,
	/*
	 * 80 
	 */ 35, 36, 13, 24, 25, 40, 41, 42, 35, 36,
	/*
	 * 90 
	 */ 37, 18, 43, 40, 41, 42, 35, 36, 19, 35,
	/*
	 * 100 
	 */ 36, 40, 41, 42, 40, 41, 47, 35, 36, 47,
	/*
	 * 110 
	 */ 47, 47, 40, 41, 20, 21, 22, 23, 47, 35,
	/*
	 * 120 
	 */ 36, 35, 47, 47, 40, 41, 40, 41,
};
...
static signed char yy_shift_ofst[] = {
	/*
	 * 0 
	 */ -5, 6, -5, -5, -5, 31, -4, 8, -3, -5,
	/*
	 * 10 
	 */ 25, -5, 23, -5, -5, -5, 48, 58, 67, 58,
	/*
	 * 20 
	 */ -5, -5, -5, -5, -5, -5, 36, 50, -5, -5,
	/*
	 * 30 
	 */ 60, -5, 58, -5, 66, 58, 67, -5, 58, 67,
	/*
	 * 40 
	 */ 65, 69, -5, 59, -5, -5, 19, 73, 58, 67,
	/*
	 * 50 
	 */ 79, 94, 58, 63, -5, -5, -5, -5, 58, -5,
	/*
	 * 60 
	 */ 58, -5, -5,
};
...
static signed char yy_reduce_ofst[] = {
	/*
	 * 0 
	 */ -2, -29, -37, -37, -37, -36, -37, -37, 3, -37,
	/*
	 * 10 
	 */ -37, 39, -29, -37, -37, -37, -37, -7, -37, 86,
	/*
	 * 20 
	 */ -37, -37, -37, -37, -37, -37, 17, -37, -37, -37,
	/*
	 * 30 
	 */ -37, -37, 53, -37, -37, 64, -37, -37, 72, -37,
	/*
	 * 40 
	 */ -37, -37, 46, -29, -37, -37, -37, -37, -21, -37,
	/*
	 * 50 
	 */ -37, 49, 84, -37, -37, -37, -37, -37, 45, -37,
	/*
	 * 60 
	 */ 61, -37, -37,
};
static YYACTIONTYPE yy_default[] = {
	/*
	 * 0 
	 */ 65, 103, 64, 66, 67, 103, 68, 103, 103, 92,
	/*
	 * 10 
	 */ 103, 65, 103, 69, 70, 71, 103, 103, 72, 103,
	/*
	 * 20 
	 */ 74, 75, 77, 78, 79, 80, 103, 86, 76, 81,
	/*
	 * 30 
	 */ 103, 82, 84, 83, 103, 103, 87, 85, 103, 73,
	/*
	 * 40 
	 */ 103, 103, 65, 103, 91, 93, 103, 103, 103, 100,
	/*
	 * 50 
	 */ 103, 103, 103, 103, 96, 97, 98, 99, 103, 101,
	/*
	 * 60 
	 */ 103, 102, 94,
};
...
/*
 * The next table maps tokens into fallback tokens.  If a construct ** like the 
 * following: ** ** %fallback ID X Y Z. ** ** appears in the grammer, then ID
 * becomes a fallback token for X, Y, ** and Z.  Whenever one of the tokens X,
 * Y, or Z is input to the parser ** but it does not parse, the type of the
 * token is changed to ID and ** the parse is retried before an error is
 * thrown. 
 */
#ifdef YYFALLBACK
...
/*
 * The following structure represents a single element of the ** parser's
 * stack.  Information stored includes: ** ** + The state number for the parser 
 * at this level of the stack. ** ** + The value of the token stored at this
 * level of the stack. ** (In other words, the "major" token.) ** ** + The
 * semantic value stored at this level of the stack.  This is ** the
 * information used by the action routines in the grammar. ** It is sometimes
 * called the "minor" token. 
 */
struct yyStackEntry {
...
/*
 * The state of the parser is completely contained in an instance of ** the
 * following structure 
 */
struct yyParser {
...
#ifndef NDEBUG
/*
 * For tracing shifts, the names of all terminals and nonterminals ** are
 * required.  The following table supplies these names 
 */
...
#ifndef NDEBUG
	if (tokenType > 0
		&& (size_t) tokenType < (sizeof(yyTokenName) / sizeof(yyTokenName[0])))
	{
...
/*
 * The following function deletes the value associated with a ** symbol.  The
 * symbol can be either a terminal or nonterminal. ** "yymajor" is the symbol
 * code, and "yypminor" is a pointer to ** the value. 
 */
static void yy_destructor(YYCODETYPE yymajor, YYMINORTYPE * yypminor)
{
	...
	{
		/*
		 * Here is inserted the actions which take place when a * terminal or
		 * non-terminal is destroyed.  This can happen * when the symbol is
		 * popped from the stack during a * reduce or during error processing
		 * or when a parser is * being destroyed before it is finished parsing.
		 * * * Note: during a reduce, the only symbols destroyed are those *
		 * which appear on the RHS of the rule, but which are not used * inside
		 * the C code. 
		 */
	case 1:
	case 2:
...
#line 556 "configparser.c"
		break;
	default:
		break;					/* If no destructor action specified: do
								 * nothing */
	}
}
 ...
 ** </ul>
 */
void configparserFree(void *p,	/* The parser to be deleted */
	...
	)
{
	...
	if (yypParser->yyidx >= YYSTACKDEPTH)
	{
		...
		/*
		 * Here code is inserted which will execute if the parser * stack every 
		 * overflows 
		 */
		configparserARG_STORE;	/* Suppress warning about unused
								 * %extra_argument var */
		return;
	}
...
/*
 * The following table contains information about every rule that ** is used
 * during the reduce. 
 */
static struct {
	YYCODETYPE lhs;				/* Symbol on the left-hand side of the rule */
	unsigned char nrhs;			/* Number of right-hand side symbols in the
								 * rule */
} yyRuleInfo[] =
{
	{
	...
	)
{
	...
	switch (yyruleno)
	{
		 ...
		 * follows: * case 0: * #line <lineno> <grammarfile> * { ... } // User
		 * supplied code * #line <lineno> <thisfile> * break; 
		 */
	case 0:
		/*
			...
			if (ctx->ok)
			{
					 ...
					 sizeof("env.") - 1) == 0)
				{
					...
					ctx->ok = 0;
						   ...
						   array_get_element(ctx->current->value,
											 yymsp[0].minor.yy41->key->ptr))
				{
					array_insert_unique(ctx->current->value,
										yymsp[0].minor.yy41);
...
#line 172 "./configparser.y"
		{
			...
			if (strncmp
				(yymsp[-2].minor.yy43->ptr, "env.", sizeof("env.") - 1) == 0)
			{
				fprintf(stderr,
				...
				ctx->ok = 0;
					   ...
					   (du =
						array_get_element(vars, yymsp[-2].minor.yy43->ptr)))
			{
				/*
				...
				yymsp[0].minor.yy41->free(yymsp[0].minor.yy41);
					   ...
					   (du =
						configparser_get_variable(ctx, yymsp[-2].minor.yy43)))
			{
				du = configparser_merge_data(du, yymsp[0].minor.yy41);
				if (NULL == du)
				{
...
#line 224 "./configparser.y"
		{
			yygotominor.yy41 =
				configparser_merge_data(yymsp[-2].minor.yy41,
										yymsp[0].minor.yy41);
...
#line 239 "./configparser.y"
		{
			...
			if (strncmp
				(yymsp[0].minor.yy43->ptr, "env.", sizeof("env.") - 1) == 0)
			{
				char *env;
				...
				} else
				{
					...
					ctx->ok = 0;
					   ...
					   (yygotominor.yy41 =
						configparser_get_variable(ctx, yymsp[0].minor.yy43)))
			{
...
#line 266 "./configparser.y"
		{
			yygotominor.yy41 = (data_unset *) data_string_init();
			buffer_copy_string_buffer(((data_string *) (yygotominor.
														yy41))->value,
									  yymsp[0].minor.yy0);
			buffer_free(yymsp[0].minor.yy0);
			yymsp[0].minor.yy0 = NULL;
...
#line 273 "./configparser.y"
		{
			...
			((data_integer *) (yygotominor.yy41))->value =
				strtol(yymsp[0].minor.yy0->ptr, NULL, 10);
			buffer_free(yymsp[0].minor.yy0);
			yymsp[0].minor.yy0 = NULL;
		}
...
#line 293 "./configparser.y"
		{
			if (buffer_is_empty(yymsp[0].minor.yy41->key) ||
				NULL == array_get_element(yymsp[-2].minor.yy40,
										  yymsp[0].minor.yy41->key->ptr))
			{
...
#line 337 "./configparser.y"
		{
			data_config *dc;
			dc = (data_config *) array_get_element(ctx->srv->
												   config_context, "global");
			assert(dc);
...
#line 355 "./configparser.y"
		{
			assert(yymsp[-3].minor.yy78->context_ndx <
				   yymsp[0].minor.yy78->context_ndx);
...
#line 380 "./configparser.y"
		{
			...
			if (NULL !=
				(dc =
				 (data_config *) array_get_element(ctx->all_configs, b->ptr)))
			{
				configparser_push(ctx, dc, 0);
			} else
			{
				...
				} comps[] =
				{
					...
					COMP_HTTP_USER_AGENT,
							CONST_STR_LEN("HTTP[\"useragent\"]")},
					{
					COMP_HTTP_USER_AGENT,
							CONST_STR_LEN("HTTP[\"user-agent\"]")},
					{
					...
					{
					COMP_HTTP_REMOTE_IP,
							CONST_STR_LEN("HTTP[\"remote-ip\"]")},
					{
					COMP_HTTP_QUERY_STRING,
							CONST_STR_LEN("HTTP[\"querystring\"]")},
					{
					COMP_HTTP_QUERY_STRING,
							CONST_STR_LEN("HTTP[\"query-string\"]")},
					{
					COMP_HTTP_REQUEST_METHOD,
							CONST_STR_LEN("HTTP[\"request-method\"]")},
					{
				...
				case CONFIG_COND_MATCH:
					{
						...
						fprintf(stderr,
								...
								yymsp[-5].minor.yy0->ptr,
								yymsp[-3].minor.yy43->ptr);
						ctx->ok = 0;
#endif
						break;
					}
...
#line 550 "./configparser.y"
		{
			...
			{
				if (0 !=
					config_parse_file(ctx->srv, ctx, yymsp[0].minor.yy43->ptr))
				{
					ctx->ok = 0;
				}
				buffer_free(yymsp[0].minor.yy43);
...
#line 560 "./configparser.y"
		{
			...
			{
				if (0 !=
					config_parse_cmd(ctx->srv, ctx, yymsp[0].minor.yy43->ptr))
				{
					ctx->ok = 0;
				}
				buffer_free(yymsp[0].minor.yy43);
...
/*
 * The main parser program. ** The first argument is a pointer to a structure
 * obtained from ** "configparserAlloc" which describes the current state of
 * the parser. ** The second argument is the major token number.  The third is
 * ** the minor token.  The fourth optional argument is whatever the ** user
 * wants (and specified in the grammar) and is available for ** use by the
 * action routines. ** ** Inputs: ** <ul> ** <li> A pointer to the parser (an
 * opaque structure.) ** <li> The major token number. ** <li> The minor token
 * number. ** <li> An option argument of a grammar-specified type. ** </ul> **
 * ** Outputs: ** None. 
 */
void configparser(void *yyp,	/* The parser */
	...
	)
{
	...
	do
	{
		...
		} else if (yyact == YY_ERROR_ACTION)
		{
...
#ifdef YYERRORSYMBOL
			/*
			 * A syntax error has occurred. * The response to an error depends
			 * upon whether or not the * grammar defines an error token
			 * "ERROR". * * This is what we do if the grammar does define
			 * ERROR: * * * Call the %syntax_error function. * * * Begin
			 * popping the stack until we enter a state where * it is legal to
			 * shift the error symbol, then shift * the error symbol. * * * Set 
			 * the error count to three. * * * Begin accepting and shifting new 
			 * tokens.  No new error * processing will occur until three tokens 
			 ...
			 * If the input token is $, then fail the parse. * * As before,
			 * subsequent error messages are suppressed until * three input
