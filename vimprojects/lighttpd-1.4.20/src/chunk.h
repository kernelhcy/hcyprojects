...
typedef struct chunk 
{
	...
	struct 
	{
		...
		struct {
			...
			off_t offset;		/* start is <n> octet away from the start
								 * of the file */
		} mmap;

		int is_temp;			/* file is temporary and will be deleted if 
								 * on cleanup */
	} file;

	off_t offset;				/* octets sent from this chunk the size of the
								 * chunk is either - mem-chunk: mem->used - 1 - 
...
int chunkqueue_append_file(chunkqueue * c, buffer * fn,
