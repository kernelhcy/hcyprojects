	...
	if (s->is_ssl)
	{
		...
		if (!buffer_is_empty(s->ssl_cipher_list))
		{
			...
			if (SSL_CTX_set_cipher_list
				(s->ssl_ctx, s->ssl_cipher_list->ptr) != 1)
			{
				log_error_write(srv, __FILE__, __LINE__, "ss", "SSL:",
								ERR_error_string(ERR_get_error(), NULL));
				return -1;
		...
		{
			if (1 !=
				SSL_CTX_load_verify_locations(s->ssl_ctx,
		...
		if (SSL_CTX_use_certificate_file
			(s->ssl_ctx, s->ssl_pemfile->ptr, SSL_FILETYPE_PEM) < 0)
		{
			log_error_write(srv, __FILE__, __LINE__, "ssb", "SSL:",
							ERR_error_string(ERR_get_error(), NULL),
							s->ssl_pemfile);
...
int network_init(server * srv)
{
	...
	for (i = 1; i < srv->config_context->used; i++)
	{
		...
		/*
		 * check if we already know this socket, if yes, don't init it 
		 */
...
#ifdef TCP_CORK
	/*
	 * Linux: put a cork into the socket as we want to combine the write()
	 * calls but only if we really have multiple chunks 
	 */
