...
static void mod_evhost_parse_pattern(plugin_config * s)
{
	...
	for (pos = ptr; *ptr; ptr++)
	{
		...
		{
			s->path_pieces =
				realloc(s->path_pieces, (s->len + 2) * sizeof(*s->path_pieces));
	...
	{
		s->path_pieces =
			realloc(s->path_pieces, (s->len + 1) * sizeof(*s->path_pieces));
		s->path_pieces[s->len] = buffer_init();
...
SETDEFAULTS_FUNC(mod_evhost_set_defaults)
{
	...
	config_values_t cv[] = {
		{"evhost.path-pattern", NULL, T_CONFIG_STRING,
...
static int mod_evhost_parse_host(connection * con, array * host)
{
	...
	char *colon = ptr;			/* needed to filter out the colon (if
								 * exists) */
	int first = 1;
	data_string *ds;
	int i;
...
#define PATCH(x) \
	p->conf.x = s->x;
static int
mod_evhost_patch_connection(server * srv, connection * con, plugin_data * p)
{
	size_t i, j;
	...
	for (i = 1; i < srv->config_context->used; i++)
	{
		...
		for (j = 0; j < dc->value->used; j++)
		{
			...
			if (buffer_is_equal_string
				(du->key, CONST_STR_LEN("evhost.path-pattern")))
			{
				PATCH(path_pieces);
				PATCH(len);
			}
...
static handler_t
mod_evhost_uri_handler(server * srv, connection * con, void *p_d)
{
	plugin_data *p = p_d;
	size_t i;
	array *parsed_host;
		...
		if (*ptr == '%')
		{
			...
			if (*(ptr + 1) == '%')
			{
				...
				buffer_append_string_len(p->tmp_buf, CONST_STR_LEN("%"));
					   ...
					   (ds =
						(data_string *) array_get_element(parsed_host,
														  p->conf.
														  path_pieces[i]->ptr)))
			{
				if (ds->value->used)
