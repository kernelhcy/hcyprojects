...
#ifdef USE_ZLIB
static int
deflate_file_to_buffer_gzip(server * srv, connection * con,
							plugin_data * p, char *start, off_t st_size,
							time_t mtime)
{
	unsigned char *c;
	unsigned long crc;
	...
	if (Z_OK != deflateInit2(&z, Z_DEFAULT_COMPRESSION, Z_DEFLATED, -MAX_WBITS,	/* supress 
																				 * zlib-header 
																				 */
...
static int
deflate_file_to_buffer_deflate(server * srv, connection * con,
...
#ifdef USE_BZ2LIB
static int
deflate_file_to_buffer_bzip2(server * srv, connection * con,
							 plugin_data * p, unsigned char *start,
							 off_t st_size)
{
	bz_stream bz;
	...
	if (BZ_OK != BZ2_bzCompressInit(&bz, 9,	/* blocksize = 900k */
									0,	/* no output */
									0))
	{							/* workFactor: default */
		return -1;
...
static int
deflate_file_to_file(server * srv, connection * con, plugin_data * p,
					 buffer * fn, stat_cache_entry * sce, int type)
{
	 ...
	 * overflow 
	 */
	if ((off_t) (sce->st.st_size * 1.1) < sce->st.st_size)
		return -1;

	/*
	 * don't mmap files > 128Mb we could use a sliding window, but currently
	 * there is no need for it 
	 */

	if (sce->st.st_size > 128 * 1024 * 1024)
		return -1;
	...
	if (0 ==
		strncmp(con->physical.path->ptr, con->physical.doc_root->ptr,
				con->physical.doc_root->used - 1))
	{
	...
	if (-1 == mkdir_for_file(p->ofn->ptr))
	{
		log_error_write(srv, __FILE__, __LINE__, "sb",
						"couldn't create directory for file", p->ofn);
		return -1;
	}

	if (-1 ==
		(ofd = open(p->ofn->ptr, O_WRONLY | O_CREAT | O_EXCL | O_BINARY, 0600)))
	{
		if (errno == EEXIST)
		{
			/*
			 ...
			 */
#if 0
			log_error_write(srv, __FILE__, __LINE__, "bs", p->ofn,
							"compress-cache hit");
#endif
		...
		log_error_write(srv, __FILE__, __LINE__, "sbss",
						"creating cachefile", p->ofn, "failed",
						strerror(errno));
	...
	{
		log_error_write(srv, __FILE__, __LINE__, "sbss",
						"opening plain-file", fn, "failed", strerror(errno));

		close(ofd);

		/*
		 * Remove the incomplete cache file, so that later hits aren't served
		 * from it 
		 */
		if (-1 == unlink(p->ofn->ptr))
		{
			log_error_write(srv, __FILE__, __LINE__, "sbss",
							"unlinking incomplete cachefile", p->ofn,
	...
	case HTTP_ACCEPT_ENCODING_GZIP:
		ret =
			deflate_file_to_buffer_gzip(srv, con, p, start,
										sce->st.st_size, sce->st.st_mtime);
		break;
	case HTTP_ACCEPT_ENCODING_DEFLATE:
		ret =
			deflate_file_to_buffer_deflate(srv, con, p, start, sce->st.st_size);
		break;
#endif
#ifdef USE_BZ2LIB
	case HTTP_ACCEPT_ENCODING_BZIP2:
	...
	if (ret == 0)
	{
		...
		{
			log_error_write(srv, __FILE__, __LINE__, "sbss",
							"writing cachefile", p->ofn, "failed:",
							strerror(errno));
			ret = -1;
		} else if ((size_t) r != p->b->used)
		{
			log_error_write(srv, __FILE__, __LINE__, "sbs",
							"writing cachefile", p->ofn,
...
static int
deflate_file_to_buffer(server * srv, connection * con, plugin_data * p,
					   buffer * fn, stat_cache_entry * sce, int type)
{
	int ifd;
	int ret = -1;
	...
	/*
	 * don't mmap files > 128M we could use a sliding window, but currently
	 * there is no need for it 
	 */
...
#define PATCH(x) \
	p->conf.x = s->x;
static int
mod_compress_patch_connection(server * srv, connection * con, plugin_data * p)
{
	size_t i, j;
	...
	for (i = 1; i < srv->config_context->used; i++)
	{
		...
		for (j = 0; j < dc->value->used; j++)
		{
			...
			if (buffer_is_equal_string
				(du->key, CONST_STR_LEN("compress.cache-dir")))
			{
				PATCH(compress_cache_dir);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("compress.filetype")))
			{
				PATCH(compress);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("compress.max-filesize")))
			{
				PATCH(compress_max_filesize);
			}
		}
...
PHYSICALPATH_FUNC(mod_compress_physical)
{
	...
	if (con->mode != DIRECT || con->http_status)
		return HANDLER_GO_ON;
	...
	/*
	 * don't try to compress files less than 128 bytes - extra overhead for
	 * compression - mmap() fails for st_size = 0 :) 
	 */
	...
	for (m = 0; m < p->conf.compress->used; m++)
	{
		...
		if (!compress_ds)
		{
			log_error_write(srv, __FILE__, __LINE__, "sbb", "evil",
							con->physical.path, con->uri.path);
