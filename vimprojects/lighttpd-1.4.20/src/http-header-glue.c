...
int http_response_redirect_to_directory(server * srv, connection * con)
{
	...
	else
	{
		...
		case AF_INET6:
			if (0 !=
				getnameinfo((const struct sockaddr *) (&our_addr.ipv6),
							SA_LEN((const struct sockaddr *) &our_addr.
								   ipv6), hbuf, sizeof(hbuf), NULL, 0, 0))
			{
		...
		case AF_INET:
			if (NULL ==
				(he =
				 gethostbyaddr((char *) &our_addr.ipv4.sin_addr,
							   sizeof(struct in_addr), AF_INET)))
			{
				log_error_write(srv, __FILE__, __LINE__,
								"SdS",
								"NOTICE: gethostbyaddr failed: ",
...
buffer *strftime_cache_get(server * srv, time_t last_mod)
{
	...
	for (i = 0; i < FILE_CACHE_MAX; i++)
	{
		 ...
		 * found cache-entry 
		 */
		if (srv->mtime_cache[i].mtime == last_mod)
...
int
http_response_handle_cachable(server * srv, connection * con, buffer * mtime)
{
	/*
	 * 14.26 If-None-Match
	 *    [...]
		...
		if (etag_is_equal(con->physical.etag, con->request.http_if_none_match))
		{
				...
				|| con->request.http_method == HTTP_METHOD_HEAD)
			{
				...
				if (con->request.http_if_modified_since)
				{
					...
					{
						used_len =
							semicolon - con->request.http_if_modified_since;
					}

					if (0 ==
						strncmp(con->request.http_if_modified_since,
								mtime->ptr, used_len))
					{
						con->http_status = 304;
					...
					{
						char
						 buf[sizeof("Sat, 23 Jul 2005 21:20:01 GMT")];
	...
	} else if (con->request.http_if_modified_since)
	{
		...
		if (NULL ==
			(semicolon = strchr(con->request.http_if_modified_since, ';')))
		{
			used_len = strlen(con->request.http_if_modified_since);
		} else
		{
