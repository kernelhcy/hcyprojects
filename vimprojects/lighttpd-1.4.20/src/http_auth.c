...
static int
http_auth_get_password(server * srv, mod_auth_plugin_data * p,
					   buffer * username, buffer * realm, buffer * password)
{
	int ret = -1;

	if (!username->used || !realm->used)
		return -1;
	...
	if (p->conf.auth_backend == AUTH_BACKEND_HTDIGEST)
	{
		...
		if (buffer_is_empty(p->conf.auth_htdigest_userfile))
			return -1;
		...
		if (0 != stream_open(&f, p->conf.auth_htdigest_userfile))
		{
			log_error_write(srv, __FILE__, __LINE__, "sbss",
							...
							p->conf.auth_htdigest_userfile, "failed:",
							strerror(errno));
		...
		while (f_line - f.start != f.size)
		{
			...
			if (NULL ==
				(f_realm = memchr(f_user, ':', f.size - (f_user - f.start))))
			{
				log_error_write(srv, __FILE__, __LINE__, "sbs",
								"parsed error in",
								p->conf.auth_htdigest_userfile,
			...
			if (NULL ==
				(f_pwd =
				 memchr(f_realm + 1, ':', f.size - (f_realm + 1 - f.start))))
			{
				log_error_write(srv, __FILE__, __LINE__, "sbs",
								"parsed error in",
			   ...
			   p->conf.auth_backend == AUTH_BACKEND_PLAIN)
	{
		...
		auth_fn =
			(p->conf.auth_backend ==
			 AUTH_BACKEND_HTPASSWD) ? p->conf.auth_htpasswd_userfile : p->
			conf.auth_plain_userfile;

		if (buffer_is_empty(auth_fn))
			return -1;

		if (0 != stream_open(&f, auth_fn))
		{
			log_error_write(srv, __FILE__, __LINE__, "sbss",
							"opening plain-userfile", auth_fn, "failed:",
							strerror(errno));
		...
		while (f_line - f.start != f.size)
		{
			...
			if (NULL ==
				(f_pwd = memchr(f_user, ':', f.size - (f_user - f.start))))
			{
				log_error_write(srv, __FILE__, __LINE__, "sbs",
...
static int
http_auth_match_rules(server * srv, mod_auth_plugin_data * p,
					  const char *url, const char *username,
					  const char *group, const char *host)
{
	...
	{
		if (p->conf.auth_require->data[i]->key->used == 0)
			continue;

		if (0 ==
			strncmp(url, p->conf.auth_require->data[i]->key->ptr,
					p->conf.auth_require->data[i]->key->used - 1))
		{
			break;
		}
	}
	...
	while (1)
	{
		...
		if (k_len == 4)
		{
			...
			{
				if (username &&
					username_len == v_len && 0 == strncmp(username, v, v_len))
				{
					return 0;
				}
...
static int
http_auth_basic_password_compare(server * srv, mod_auth_plugin_data * p,
								 array * req, buffer * username,
								 buffer * realm, buffer * password,
								 const char *pw)
{
	UNUSED(srv);
	UNUSED(req);
	...
	if (p->conf.auth_backend == AUTH_BACKEND_HTDIGEST)
	{
		...
		MD5_Init(&Md5Ctx);
		MD5_Update(&Md5Ctx, (unsigned char *) username->ptr,
				   username->used - 1);
	...
	} else if (p->conf.auth_backend == AUTH_BACKEND_LDAP)
	{
		...
		/*
		 * check username we have to protect us againt username which modifies 
		 * out filter in a unpleasant way 
		 */
		...
		if (p->conf.ldap == NULL ||
			LDAP_SUCCESS != (ret =
							 ldap_search_s(p->conf.ldap,
										   p->conf.auth_ldap_basedn->ptr,
										   LDAP_SCOPE_SUBTREE,
										   p->ldap_filter->ptr, attrs, 0, &lm)))
		{
			/*
			 * try again if ldap was only temporary down 
			 */
			if (p->conf.ldap == NULL || ret != LDAP_SERVER_DOWN
				|| LDAP_SUCCESS != (ret =
									ldap_search_s(p->conf.ldap,
												  p->conf.
												  auth_ldap_basedn->ptr,
												  LDAP_SCOPE_SUBTREE,
												  p->ldap_filter->ptr,
												  attrs, 0, &lm)))
			{
				...
				ldap = p->conf.ldap;	/* save temporary ldap connection
										 * (TODO: redo ldap) */
				if (LDAP_SUCCESS !=
					(ret =
					 ldap_search_s(p->conf.ldap,
								   ...
								   p->ldap_filter->ptr, attrs, 0, &lm)))
				{
					log_error_write(srv, __FILE__, __LINE__, "sssb",
									"ldap:", ldap_err2string(ret),
									"filter:", p->ldap_filter);
					/*
		 ...
		 */
		if (NULL ==
			(ldap = ldap_init(p->conf.auth_ldap_hostname->ptr, LDAP_PORT)))
		{
		...
		if (p->conf.auth_ldap_starttls == 1)
		{
			...
			{
				log_error_write(srv, __FILE__, __LINE__, "ss",
								"ldap startTLS failed:", ldap_err2string(ret));
...
int
http_auth_basic_check(server * srv, connection * con,
					  mod_auth_plugin_data * p, array * req, buffer * url,
					  const char *realm_str)
{
	...
	{
		log_error_write(srv, __FILE__, __LINE__, "sb",
						"decodeing base64-string failed", username);
	...
	{
		log_error_write(srv, __FILE__, __LINE__, "sb", ": is missing in",
						username);
	...
	*pw++ = '\0';
	 ...
	 */
	if (http_auth_basic_password_compare
		(srv, p, req, username, realm->value, password, pw))
	{
		log_error_write(srv, __FILE__, __LINE__, "sbbss",
						"password doesn't match for ", con->uri.path,
						username, ", IP:", inet_ntop_cache_get_ip(srv,
																  &(con->
																	dst_addr)));
...
int
http_auth_digest_check(server * srv, connection * con,
					   mod_auth_plugin_data * p, array * req, buffer * url,
					   const char *realm_str)
{
	char a1[256];
...
#undef S
	 ...
	 */
	if (!username ||
		!realm || !nonce || !uri || (qop && (!nc || !cnonce)) || !respons)
	{
		/*
		 * missing field 
		 */

		log_error_write(srv, __FILE__, __LINE__, "s", "digest: missing field");
	 ...
	 */
	if (algorithm &&
		0 == strcasecmp(algorithm, "md5-sess") && (!nonce || !cnonce))
	{
		log_error_write(srv, __FILE__, __LINE__, "s",
	...
	if (p->conf.auth_backend == AUTH_BACKEND_PLAIN)
	{
		...
		MD5_Update(&Md5Ctx, (unsigned char *) ":", 1);
		MD5_Update(&Md5Ctx, (unsigned char *) password->ptr,
				   password->used - 1);
		MD5_Final(HA1, &Md5Ctx);
	...
	if (algorithm && strcasecmp(algorithm, "md5-sess") == 0)
	{
		MD5_Init(&Md5Ctx);
	...
	if (0 != strcmp(a2, respons))
	{
		...
		log_error_write(srv, __FILE__, __LINE__, "sss",
						"digest: auth failed for ", username,
						": wrong password, IP:",
	...
	if (p->conf.auth_debug)
	{
		log_error_write(srv, __FILE__, __LINE__, "s", "digest: auth ok");
	}
	return 1;
}
...
int
http_auth_digest_generate_nonce(server * srv, mod_auth_plugin_data * p,
								buffer * fn, char out[33])
{
	HASH h;
	MD5_CTX Md5Ctx;
	char hh[32];
