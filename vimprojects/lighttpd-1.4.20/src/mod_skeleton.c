...
SETDEFAULTS_FUNC(mod_skeleton_set_defaults)
{
	...
	config_values_t cv[] = {
		{"skeleton.array", NULL, T_CONFIG_ARRAY, T_CONFIG_SCOPE_CONNECTION}
		,						/* 0 */
...
#define PATCH(x) \
	p->conf.x = s->x;
static int
mod_skeleton_patch_connection(server * srv, connection * con, plugin_data * p)
{
	size_t i, j;
	...
	for (i = 1; i < srv->config_context->used; i++)
	{
		...
		for (j = 0; j < dc->value->used; j++)
		{
			...
			if (buffer_is_equal_string
				(du->key, CONST_STR_LEN("skeleton.array")))
			{
				PATCH(match);
			}
		}
...
URIHANDLER_FUNC(mod_skeleton_uri_handler)
{
	...
	for (k = 0; k < p->conf.match->used; k++)
	{
		...
		if (0 ==
			strncmp(con->uri.path->ptr + s_len - ct_len, ds->value->ptr,
					ct_len))
		{
			con->http_status = 403;
