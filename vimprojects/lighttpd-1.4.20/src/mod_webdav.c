...
SETDEFAULTS_FUNC(mod_webdav_set_defaults)
{
	...
	config_values_t cv[] = {
		{"webdav.activate", NULL, T_CONFIG_BOOLEAN,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 0 */
		{"webdav.is-readonly", NULL, T_CONFIG_BOOLEAN,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 1 */
		{"webdav.sqlite-db-name", NULL, T_CONFIG_STRING,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 2 */
		{"webdav.log-xml", NULL, T_CONFIG_BOOLEAN,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 3 */
		...
		if (!buffer_is_empty(s->sqlite_db_name))
		{
			...
			if (SQLITE_OK != sqlite3_open(s->sqlite_db_name->ptr, &(s->sql)))
			{
				log_error_write(srv, __FILE__, __LINE__, "sbs",
								"sqlite3_open failed for",
								s->sqlite_db_name, sqlite3_errmsg(s->sql));
				return HANDLER_ERROR;
			}
			...
			if (SQLITE_OK != sqlite3_prepare(s->sql,
											 CONST_STR_LEN
											 ("SELECT ns, prop FROM properties WHERE resource = ?"),
											 &(s->stmt_select_propnames),
											 &next_stmt))
			{
				/*
				 * prepare failed 
			...
			if (SQLITE_OK != sqlite3_prepare(s->sql,
											 CONST_STR_LEN
											 ("INSERT INTO properties SELECT ?, prop, ns, value FROM properties WHERE resource = ?"),
			...
			if (SQLITE_OK != sqlite3_prepare(s->sql,
											 CONST_STR_LEN
											 ("UPDATE properties SET resource = ? WHERE resource = ?"),
			...
			if (SQLITE_OK != sqlite3_prepare(s->sql,
											 CONST_STR_LEN
											 ("INSERT INTO locks (locktoken, resource, lockscope, locktype, owner, depth, timeout) VALUES (?,?,?,?,?,?, CURRENT_TIME + 600)"),
											 &(s->stmt_create_lock),
											 &next_stmt))
			{
				/*
			...
			if (SQLITE_OK != sqlite3_prepare(s->sql,
											 CONST_STR_LEN
											 ("SELECT locktoken, resource, lockscope, locktype, owner, depth, timeout FROM locks WHERE locktoken = ?"),
			...
			if (SQLITE_OK != sqlite3_prepare(s->sql,
											 CONST_STR_LEN
											 ("SELECT locktoken, resource, lockscope, locktype, owner, depth, timeout FROM locks WHERE resource = ?"),
											 &(s->stmt_read_lock_by_uri),
											 &next_stmt))
			{
				/*
				 * prepare failed 
			...
			if (SQLITE_OK != sqlite3_prepare(s->sql,
											 CONST_STR_LEN
											 ("UPDATE locks SET timeout = CURRENT_TIME + 600 WHERE locktoken = ?"),
											 &(s->stmt_refresh_lock),
											 &next_stmt))
			{
				/*
...
#define PATCH_OPTION(x) \
	p->conf.x = s->x;
static int
mod_webdav_patch_connection(server * srv, connection * con, plugin_data * p)
{
	size_t i, j;
	...
	for (i = 1; i < srv->config_context->used; i++)
	{
		...
		for (j = 0; j < dc->value->used; j++)
		{
			...
			if (buffer_is_equal_string
				(du->key, CONST_STR_LEN("webdav.activate")))
			{
				PATCH_OPTION(enabled);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("webdav.is-readonly")))
			{
				PATCH_OPTION(is_readonly);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("webdav.log-xml")))
			{
				PATCH_OPTION(log_xml);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("webdav.sqlite-db-name")))
			{
#ifdef USE_PROPPATCH
				PATCH_OPTION(sql);
				PATCH_OPTION(stmt_update_prop);
...
URIHANDLER_FUNC(mod_webdav_uri_handler)
{
	...
	case HTTP_METHOD_OPTIONS:
		/*
		 * we fake a little bit but it makes MS W2k happy and it let's us mount 
		 * the volume 
		 */
		response_header_overwrite(srv, con, CONST_STR_LEN("DAV"),
								  CONST_STR_LEN("1,2"));
...
static int
webdav_gen_prop_tag(server * srv, connection * con,
					char *prop_name, char *prop_ns, char *value, buffer * b)
{
...
static int
webdav_gen_response_status_tag(server * srv, connection * con,
							   physical * dst, int status, buffer * b)
{
	UNUSED(srv);
...
static int
webdav_delete_file(server * srv, connection * con, plugin_data * p,
				   physical * dst, buffer * b)
{
	int status = 0;
...
static int
webdav_delete_dir(server * srv, connection * con, plugin_data * p,
				  physical * dst, buffer * b)
{
	DIR *dir;
...
static int
webdav_copy_file(server * srv, connection * con, plugin_data * p,
				 physical * src, physical * dst, int overwrite)
{
	stream s;
	int status = 0, ofd;
	UNUSED(srv);
	...
	if (-1 ==
		(ofd =
		 open(dst->path->ptr,
			  O_WRONLY | O_TRUNC | O_CREAT | (overwrite ? 0 : O_EXCL),
			  WEBDAV_FILE_MODE)))
	{
		/*
...
static int
webdav_copy_dir(server * srv, connection * con, plugin_data * p,
				physical * src, physical * dst, int overwrite)
{
	DIR *srcdir;
	int status = 0;
		...
		while (NULL != (de = readdir(srcdir)))
		{
			...
			} else if (S_ISDIR(st.st_mode))
			{
				...
				} else
				{
					...
					if (0 !=
						(status =
						 webdav_copy_dir(srv, con, p, &s, &d, overwrite)))
					{
						break;
					}
					/*
			...
			} else if (S_ISREG(st.st_mode))
			{
				 ...
				 */
				if (0 !=
					(status = webdav_copy_file(srv, con, p, &s, &d, overwrite)))
				{
					break;
				}
			}
...
static int
webdav_get_live_property(server * srv, connection * con, plugin_data * p,
						 physical * dst, char *prop_name, buffer * b)
{
	...
	if (HANDLER_ERROR != (stat_cache_get_entry(srv, con, dst->path, &sce)))
	{
		...
		} else if (0 == strcmp(prop_name, "getcontenttype"))
		{
			...
			} else if (S_ISREG(sce->st.st_mode))
			{
				...
				{
					data_string *ds =
						(data_string *) con->conf.mimetypes->data[k];

					if (ds->key->used == 0)
						continue;

					if (buffer_is_equal_right_len
						(dst->path, ds->key, ds->key->used - 1))
					{
						buffer_append_string_len(b,
												 CONST_STR_LEN
												 ("<D:getcontenttype>"));
		...
		} else if (0 == strcmp(prop_name, "creationdate"))
		{
			...
			strftime(ctime_buf, sizeof(ctime_buf), "%Y-%m-%dT%H:%M:%SZ",
					 gmtime(&(sce->st.st_ctime)));
		...
		} else if (0 == strcmp(prop_name, "getlastmodified"))
		{
			...
			strftime(mtime_buf, sizeof(mtime_buf),
					 "%a, %d %b %Y %H:%M:%S GMT", gmtime(&(sce->st.st_mtime)));
		...
		{
			buffer_append_string_len(b,
									 CONST_STR_LEN("<D:getcontentlanguage>"));
...
static int
webdav_get_property(server * srv, connection * con, plugin_data * p,
					physical * dst, char *prop_name, char *prop_ns, buffer * b)
{
	...
	} else
	{
		...
		if (stmt)
		{
			...
			sqlite3_bind_text(stmt, 1,
							  ...
							  dst->rel_path->used - 1, SQLITE_TRANSIENT);
			sqlite3_bind_text(stmt, 2,
							  prop_name, strlen(prop_name), SQLITE_TRANSIENT);
			sqlite3_bind_text(stmt, 3, prop_ns, strlen(prop_ns),
							  SQLITE_TRANSIENT);
			...
			while (SQLITE_ROW == sqlite3_step(stmt))
			{
				 ...
				 */
				webdav_gen_prop_tag(srv, con, prop_name, prop_ns,
									(char *) sqlite3_column_text(stmt, 0), b);
				found = 1;
			}
		}
#endif
...
static int
webdav_get_props(server * srv, connection * con, plugin_data * p,
				 physical * dst, webdav_properties * props, buffer * b_200,
				 buffer * b_404)
{
	size_t i;
	...
	if (props)
	{
		for (i = 0; i < props->used; i++)
		{
			...
			{
				webdav_gen_prop_tag(srv, con, prop->prop, prop->ns,
									NULL, b_404);
			}
		}
	} else
	{
		for (i = 0; live_properties[i].prop; i++)
		{
			 ...
			 * a local 'live' property 
			 */
			webdav_get_live_property(srv, con, p, dst,
									 live_properties[i].prop, b_200);
		}
	}
...
#ifdef USE_PROPPATCH
static int
webdav_parse_chunkqueue(server * srv, connection * con, plugin_data * p,
						chunkqueue * cq, xmlDoc ** ret_xml)
{
	...
	for (c = cq->first; cq->bytes_out != cq->bytes_in; c = cq->first)
	{
		...
		case FILE_CHUNK:
			...
			if (c->file.mmap.start == MAP_FAILED)
			{
				...
				if (MAP_FAILED ==
					(c->file.mmap.start =
					 mmap(0, c->file.length, PROT_READ, MAP_SHARED,
						  c->file.fd, 0)))
				{
			...
			if (XML_ERR_OK !=
				(err =
				 xmlParseChunk(ctxt, c->file.mmap.start + c->offset,
							   weHave, 0)))
			{
				log_error_write(srv, __FILE__, __LINE__, "sodd",
								"xmlParseChunk failed at:",
								cq->bytes_out, weHave, err);
			}
		...
		case MEM_CHUNK:
			...
			{
				log_error_write(srv, __FILE__, __LINE__, "ss",
								"XML-request-body:", c->mem->ptr + c->offset);
			}

			if (XML_ERR_OK !=
				(err = xmlParseChunk(ctxt, c->mem->ptr + c->offset, weHave, 0)))
			{
				log_error_write(srv, __FILE__, __LINE__, "sodd",
								"xmlParseChunk failed at:", cq->bytes_out,
								weHave, err);
			}
	...
	default:
		log_error_write(srv, __FILE__, __LINE__, "sd",
						"xmlParseChunk failed at final packet:", err);
		break;
	}
...
int
webdav_lockdiscovery(server * srv, connection * con,
					 buffer * locktoken, const char *lockscope,
					 const char *locktype, int depth)
{

	buffer *b;

	response_header_overwrite(srv, con, CONST_STR_LEN("Lock-Token"),
 ...
 */
int
webdav_has_lock(server * srv, connection * con, plugin_data * p, buffer * uri)
{
	int has_lock = 1;
	 ...
	 */
	if (NULL !=
		(ds = (data_string *) array_get_element(con->request.headers, "If")))
	{
		/*
		 * Ooh, ooh. A if tag, now the fun begins. this can only work with a
		 * real parser 
		 */
	} else
	{
		...
		sqlite3_bind_text(stmt, 1, CONST_BUF_LEN(uri), SQLITE_TRANSIENT);
...
URIHANDLER_FUNC(mod_webdav_subrequest_handler)
{
	 ...
	 */
	if (NULL !=
		(ds = (data_string *) array_get_element(con->request.headers, "Depth")))
	{
	...
	case HTTP_METHOD_PROPFIND:
		...
		if (con->request.content_length)
		{
			...
			if (1 ==
				webdav_parse_chunkqueue(srv, con, p,
										con->request_content_queue, &xml))
			{
					...
					for (cmd = rootnode->children; cmd; cmd = cmd->next)
					{
						...
						if (0 == xmlStrcmp(cmd->name, BAD_CAST "prop"))
						{
							...
							for (prop = cmd->children; prop; prop = prop->next)
							{
								...
								req_props->ptr[req_props->
											   used] =
									malloc(sizeof(webdav_property));
								req_props->ptr[req_props->used]->
									ns =
									(char *) xmlStrdup(prop->
													   ns ? prop->
													   ns->
													   href : (xmlChar *) "");
								req_props->ptr[req_props->used]->
									prop = (char *) xmlStrdup(prop->name);
								req_props->used++;
							}
						} else if (0 ==
								   xmlStrcmp(cmd->name, BAD_CAST "propname"))
						{
							...
							if (stmt)
							{
								...
								if (SQLITE_DONE != sqlite3_step(stmt))
								{
								   ...
								   xmlStrcmp(cmd->name, BAD_CAST "allprop"))
						{
							/*
		...
		case 0:
			 ...
			 * Depth: 0 
			 */
			webdav_get_props(srv, con, p, &(con->physical), req_props,
							 prop_200, prop_404);
		...
		case 1:
			if (NULL != (dir = opendir(con->physical.path->ptr)))
			{
				...
				while (NULL != (de = readdir(dir)))
				{
					...
					webdav_get_props(srv, con, p, &d, req_props,
									 prop_200, prop_404);

					buffer_append_string_len(b,
											 CONST_STR_LEN("<D:response>\n"));
					buffer_append_string_len(b, CONST_STR_LEN("<D:href>"));
					buffer_append_string_buffer(b, con->uri.scheme);
					buffer_append_string_len(b, CONST_STR_LEN("://"));
					buffer_append_string_buffer(b, con->uri.authority);
	...
	case HTTP_METHOD_DELETE:
		...
		} else if (S_ISDIR(st.st_mode))
		{
			...
			if (webdav_delete_dir
				(srv, con, p, &(con->physical), multi_status_resp))
			{
				/*
	...
	case HTTP_METHOD_PUT:
		{
			...
			/*
			 * RFC2616 Section 9.6 PUT requires us to send 501 on all Content-* 
			 * we don't support - most important Content-Range Example:
			 * Content-Range: bytes 100-1037/1038 
			 */
												   ...
												   "Content-Range")))
			{
				...
				if (-1 ==
					(fd =
					 open(con->physical.path->ptr, O_WRONLY, WEBDAV_FILE_MODE)))
				{
					switch (errno)
					{
					case ENOENT:
			...
			} else
			{
				...
				if (-1 ==
					(fd =
					 open(con->physical.path->ptr, O_WRONLY | O_TRUNC,
						  WEBDAV_FILE_MODE)))
				{
					if (errno == ENOENT &&
						-1 == (fd =
							   open(con->physical.path->ptr,
									O_WRONLY | O_CREAT | O_TRUNC |
									O_EXCL, WEBDAV_FILE_MODE)))
					{
						/*
			...
			for (c = cq->first; c; c = cq->first)
			{
				...
				case FILE_CHUNK:
					...
					if (c->file.mmap.start == MAP_FAILED)
					{
						if (-1 == c->file.fd &&	/* open the file if not already 
												 * open */
							-1 == (c->file.fd =
						...
						if (MAP_FAILED ==
							(c->file.mmap.start =
							 mmap(0, c->file.length, PROT_READ,
								  MAP_SHARED, c->file.fd, 0)))
						{
							log_error_write(srv, __FILE__,
											__LINE__, "ssbd",
											"mmap failed: ",
					...
					if ((r =
						 write(fd, c->file.mmap.start + c->offset,
	...
	case HTTP_METHOD_COPY:
		{
			...
			{
				if (ds->value->used != 2 ||
					(ds->value->ptr[0] != 'F' && ds->value->ptr[0] != 'T'))
				{
					con->http_status = 400;
					return HANDLER_FINISHED;
				}
			 ...
			 * http://127.0.0.1:1025/dav/litmus/copydest - host has to be the
			 * same as the Host: header we got - we have to stay inside the
			 * document root - the query string is thrown away 
			 */
			...
			/*
			 * we now have a URI which is clean. transform it into a physical
			 * path 
			 */
			buffer_copy_string_buffer(p->physical.doc_root,
			...
			{
				buffer_append_string_buffer(p->physical.path,
											p->physical.rel_path);
			}

			/*
			 * let's see if the source is a directory if yes, we fail with 501 
			 */
			...
			} else if (S_ISDIR(st.st_mode))
			{
				...
				if (0 !=
					(r =
					 webdav_copy_dir(srv, con, p, &(con->physical),
									 &(p->physical), overwrite)))
				{
					con->http_status = r;
					return HANDLER_FINISHED;
				}
			...
			} else
			{
				...
				if (0 == (r = stat(p->physical.path->ptr, &st)))
				{
					...
					{
						/*
						 * file to dir/ append basename to physical path 
						 */

						if (NULL !=
							(sep = strrchr(con->physical.path->ptr, '/')))
						{
				...
				if (con->request.http_method == HTTP_METHOD_MOVE)
				{
					...
					if (0 ==
						rename(con->physical.path->ptr, p->physical.path->ptr))
					{
#ifdef USE_PROPPATCH
				...
				if (0 !=
					(r =
					 webdav_copy_file(srv, con, p, &(con->physical),
									  &(p->physical), overwrite)))
				{
					con->http_status = r;
		...
		if (con->request.content_length)
		{
										...
										con->request_content_queue, &xml))
			{
				...
				if (0 == xmlStrcmp(rootnode->name, BAD_CAST "propertyupdate"))
				{
					xmlNode *cmd;
					char *err = NULL;
					int empty_ns = 0;	/* send 400 on a empty
										 * namespace attribute */
					...
					if (SQLITE_OK !=
						sqlite3_exec(p->conf.sql,
									 "BEGIN TRANSACTION", NULL, NULL, &err))
					{
						log_error_write(srv, __FILE__, __LINE__, "ss",
										"can't open transaction:", err);
						sqlite3_free(err);
					...
					for (cmd = rootnode->children; cmd; cmd = cmd->next)
					{
						...
						if ((0 ==
							 xmlStrcmp(cmd->name, BAD_CAST "set"))
							|| (0 == xmlStrcmp(cmd->name, BAD_CAST "remove")))
						{
							...
							stmt =
								(0 ==
								 xmlStrcmp(cmd->name,
										   BAD_CAST "remove")) ?
								p->conf.stmt_delete_prop : p->
								conf.stmt_update_prop;

							for (props = cmd->children; props;
								 props = props->next)
							{
								if (0 ==
									xmlStrcmp(props->name, BAD_CAST "prop"))
								{
									xmlNode *prop;
									int r;
									...
									if (prop->ns)
									{
										sqlite3_bind_text
											(stmt, 3,
											 ...
											 strlen((char *)
													prop->ns->
													href), SQLITE_TRANSIENT);
									} else
									{
										sqlite3_bind_text
											(stmt, 3, "", 0, SQLITE_TRANSIENT);
									}
					...
					{
						if (SQLITE_OK !=
							sqlite3_exec(p->conf.sql, "ROLLBACK",
										 NULL, NULL, &err))
						{
							log_error_write(srv, __FILE__,
											__LINE__, "ss",
											"can't rollback transaction:", err);
							sqlite3_free(err);
					...
					{
						if (SQLITE_OK !=
							sqlite3_exec(p->conf.sql, "COMMIT",
										 NULL, NULL, &err))
						{
							log_error_write(srv, __FILE__,
											__LINE__, "ss",
											"can't commit transaction:", err);
							sqlite3_free(err);
	...
	case HTTP_METHOD_LOCK:
		...
		if (con->request.content_length)
		{
			...
			if (NULL !=
				(ds =
				 (data_string *) array_get_element(con->request.headers, "If")))
			{
				hdr_if = ds->value;
			}
										...
										con->request_content_queue, &xml))
			{
				...
				{
					xmlNode *lockinfo;
					const xmlChar *lockscope = NULL, *locktype = NULL;	/* TODO: compiler says unused:
																		 * *owner = NULL; */

					for (lockinfo = rootnode->children; lockinfo;
						 lockinfo = lockinfo->next)
					{
						if (0 ==
							xmlStrcmp(lockinfo->name, BAD_CAST "lockscope"))
						{
							xmlNode *value;
							for (value = lockinfo->children;
								 value; value = value->next)
							{
								if ((0 ==
									 xmlStrcmp(value->name,
											   BAD_CAST
											   "exclusive"))
									|| (0 ==
										xmlStrcmp(value->name,
											 ...
											 BAD_CAST "locktype"))
						{
							...
							{
								if ((0 ==
									 xmlStrcmp(value->name, BAD_CAST "write")))
								{
									locktype = value->name;
								} else
								{
								   ...
								   xmlStrcmp(lockinfo->name, BAD_CAST "owner"))
						{
						}
					}
					...
					if (lockscope && locktype)
					{
						...
						/*
						 * SELECT locktoken, resource, lockscope, locktype,
						 * owner, depth, timeout FROM locks WHERE resource = ? 
						 */
						...
						if (stmt)
						{
							...
							{
								/*
								 * we found a lock 1. is it compatible ? 2. is
								 * it ours 
								 */
								char *sql_lockscope =
						...
						if (stmt)
						{
							...
							/*
							 * "CREATE TABLE locks (" " locktoken TEXT NOT
							 * NULL," " resource TEXT NOT NULL," " lockscope
							 * TEXT NOT NULL," " locktype TEXT NOT NULL," "
							 * owner TEXT NOT NULL," " depth INT NOT NULL," 
							 */
		...
		} else
		{
				 ...
				 (data_string *) array_get_element(con->request.headers, "If")))
			{
				...
				buffer_copy_string_len(p->tmp_buf, locktoken->ptr + 2,
									   locktoken->used - 5);

				sqlite3_reset(stmt);

				sqlite3_bind_text(stmt, 1,
								  CONST_BUF_LEN(p->tmp_buf), SQLITE_TRANSIENT);
				...
				webdav_lockdiscovery(srv, con, p->tmp_buf, "exclusive",
									 "write", 0);
	...
	case HTTP_METHOD_UNLOCK:
											   ...
											   "Lock-Token")))
		{
			...
			buffer_copy_string_len(p->tmp_buf, locktoken->ptr + 1,
								   locktoken->used - 3);
			...
			sqlite3_bind_text(stmt, 2,
							  CONST_BUF_LEN(con->uri.path), SQLITE_TRANSIENT);
