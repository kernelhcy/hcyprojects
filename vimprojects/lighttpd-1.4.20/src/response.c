...
int http_response_write_header(server * srv, connection * con)
{
	...
	for (i = 0; i < con->response.headers->used; i++)
	{
		...
		{
			if (0 == strcasecmp(ds->key->ptr, "Date"))
				have_date = 1;
			if (0 == strcasecmp(ds->key->ptr, "Server"))
				have_server = 1;
			if (0 == strcasecmp(ds->key->ptr, "Content-Encoding")
				&& 304 == con->http_status)
				continue;
			 ...
			 */
			buffer_append_string_encoded(b, CONST_BUF_LEN(ds->value),
...
handler_t http_response_prepare(server * srv, connection * con)
{
	...
	if (con->mode == DIRECT && con->physical.path->used == 0)
	{
		...
		/*
		 * we only come here when we have the parse the full request again a
		 * HANDLER_COMEBACK from mod_rewrite and mod_fastcgi might be a problem 
		 * here as mod_setenv might get called multiple times fastcgi-auth
		 * might lead to a COMEBACK too fastcgi again dead server too
		...
		/*
		 * strip dots from the end and spaces windows/dos handle those
		 * filenames as the same file foo == foo. == foo..... == "foo...  " == 
		 * "foo..  ./" This will affect in some cases PATHINFO on native
		 * windows we could prepend the filename with \\?\ to circumvent this
		 * behaviour. I have no idea how to push this through cygwin 
		 */
		 ...
		 * physical.path for us (all vhost-plugins are supposed to set the
		 * doc_root) 
		 */
		...
		/*
		 * the docroot plugins might set the servername, if they don't we take
		 * http-host 
		 */
	...
	if (con->mode == DIRECT)
	{
		...
		/*
		 * if we are still here, no one wanted the file, status 403 is ok I
		 * think 
		 */
