...
int connection_close(server * srv, connection * con)
{
	...
	srv->cur_fds--;
#if 0
	log_error_write(srv, __FILE__, __LINE__, "sd", "closed()", con->fd);
#endif
...
int connection_handle_read_state(server * srv, connection * con)
{
			...
			for (i = 0; !last_chunk && i < b.used; i++)
			{
				...
				case '\r':
					...
					else
					{
						...
						/*
						 * looks like the following chars are not in the same
						 * chunk 
						 */
	...
	case CON_STATE_READ_POST:
			 		...
			 		c = c->next)
		{
			...
			if (con->request.content_length > 64 * 1024)
			{
				...
				{
					/*
					 * we don't have file to write to, EACCES might be one
					 * reason. Instead of sending 500 we send 413 and say the
	 ...
	 * the READ states the only way is to leave here 
	 */
...
handler_t connection_handle_fdevent(void *s, void *context, int revents)
{
	...
	if (revents & FDEVENT_IN)
	{
		con->is_readable = 1;
#if 0
		log_error_write(srv, __FILE__, __LINE__, "sd", "read-wait - done",
						con->fd);
#endif
	}
...
connection *connection_accept(server * srv, server_socket * srv_socket)
{
	...
	else
	{
		 ...
		 */
#if 0
		log_error_write(srv, __FILE__, __LINE__, "sd", "appected()", cnt);
#endif
		srv->con_opened++;
...
int connection_state_machine(server * srv, connection * con)
{
		...
		case CON_STATE_RESPONSE_END:	/* transient */
			...
			else
			{
				...
				if (srv_sock->is_ssl)
				{
					...
					case 0:
						/*
						 * wait for fd-event FIXME: wait for fdevent and call
						 * SSL_shutdown again 
						 */
		...
		case CON_STATE_WRITE:
			...
			if (!chunkqueue_is_empty(con->write_queue))
			{
				...
				log_error_write(srv, __FILE__, __LINE__, "dsd",
								con->fd,
								"packets to write:", con->write_queue->used);
#endif
			}
					...
					case SSL_ERROR_SYSCALL:
						...
						} else
						{
							log_error_write(srv, __FILE__,
											__LINE__, "sddds",
											"SSL (error):",
											ssl_r, ret, errno, strerror(errno));
						}
	...
	case CON_STATE_WRITE:
		/*
		 * request write-fdevent only if we really need it - if we have data to 
		 * write - if the socket is not writable yet 
		 */
		if (!chunkqueue_is_empty(con->write_queue) &&
