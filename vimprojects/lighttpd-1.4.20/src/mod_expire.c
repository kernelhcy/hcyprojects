...
static int
mod_expire_get_offset(server * srv, plugin_data * p, buffer * expire,
					  int *offset)
{
	char *ts;
	int type = -1;
	...
	if (expire->used == 0)
	{
		log_error_write(srv, __FILE__, __LINE__, "s", "empty:");
		return -1;
	}
	...
	} else
	{
		 ...
		 * invalid type-prefix 
		 */
		log_error_write(srv, __FILE__, __LINE__, "ss", "invalid <base>:", ts);
		return -1;
	}
	...
	while (1)
	{
		...
		if (NULL != (space = strchr(ts, ' ')))
		{
			...
			if (slen == 5 && 0 == strncmp(ts, "years", slen))
			{
				num *= 60 * 60 * 24 * 30 * 12;
			} else if (slen == 6 && 0 == strncmp(ts, "months", slen))
			{
				num *= 60 * 60 * 24 * 30;
			} else if (slen == 5 && 0 == strncmp(ts, "weeks", slen))
			{
				num *= 60 * 60 * 24 * 7;
			} else if (slen == 4 && 0 == strncmp(ts, "days", slen))
			{
				num *= 60 * 60 * 24;
			} else if (slen == 5 && 0 == strncmp(ts, "hours", slen))
			{
				num *= 60 * 60;
			} else if (slen == 7 && 0 == strncmp(ts, "minutes", slen))
			{
				num *= 60;
			} else if (slen == 7 && 0 == strncmp(ts, "seconds", slen))
			{
				num *= 1;
			} else
			{
...
SETDEFAULTS_FUNC(mod_expire_set_defaults)
{
	...
	config_values_t cv[] = {
		{"expire.url", NULL, T_CONFIG_ARRAY, T_CONFIG_SCOPE_CONNECTION}
		,						/* 0 */
...
#define PATCH(x) \
	p->conf.x = s->x;
static int
mod_expire_patch_connection(server * srv, connection * con, plugin_data * p)
{
	size_t i, j;
...
URIHANDLER_FUNC(mod_expire_path_handler)
{
	...
	for (k = 0; k < p->conf.expire_url->used; k++)
	{
		...
		if (0 == strncmp(con->uri.path->ptr, ds->key->ptr, ct_len))
		{
			...
			if (0 ==
				(len =
				 strftime(p->expire_tstmp->ptr,
						  p->expire_tstmp->size - 1,
