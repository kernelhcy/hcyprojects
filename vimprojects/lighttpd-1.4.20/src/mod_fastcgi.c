...
typedef struct fcgi_proc {
	...
	buffer *connection_name;	/* either tcp:<host>:<port> or
								 * unix:<socket> for debugging purposes 
								 */

	pid_t pid;					/* PID of the spawned process (0 if not spawned 
								 * locally) */
	...
	time_t disabled_until;		/* this proc is disabled until, use
	...
	enum {
		...
		PROC_STATE_OVERLOADED,	/* listen-queue is full, don't send anything to 
								 * this proc for the next 2 seconds */
...
typedef struct {
	...
	/*
	 * list of processes handling this extension sorted by lowest load
	 * whenever a job is done move it up in the list until it is sorted, move
	 * it down as soon as the job is started 
	 */
	fcgi_proc *first;
	...
	/*
	 * if socket is local we can start the fastcgi process ourself bin-path is 
	 * the path to the binary check min_procs and max_procs for the number of
	 * process to start up 
	 */
	buffer *bin_path;

	/*
	 * bin-path is set bin-environment is taken to create the environement
	 * before starting the FastCGI process 
	 */
	array *bin_env;
	...
	size_t max_id;				/* corresponds most of the time to num_procs.
								 * 
								 * only if a process is killed max_id waits for 
								 * the process itself to die and decrements it
								 * afterwards */
...
typedef struct {
	...
	plugin_config conf;			/* this is only used as long as no handler_ctx
								 * is setup */
} plugin_data;
...
int
fastcgi_status_copy_procname(buffer * b, fcgi_extension_host * host,
							 fcgi_proc * proc)
{
...
int
fastcgi_status_init(server * srv, buffer * b, fcgi_extension_host * host,
					fcgi_proc * proc)
{
#define CLEAN(x) \
...
int
fastcgi_extension_insert(fcgi_exts * ext, buffer * key,
...
static int
env_add(char_array * env, const char *key, size_t key_len, const char *val,
		size_t val_len)
{
	char *dst;
	size_t i;
	...
	for (i = 0; i < env->used; i++)
	{
		...
		{
			/*
			 * don't care about free as we are in a forked child which is going 
			 * to exec(...) 
			 */
			/*
			 * free(env->ptr[i]); 
...
static int
fcgi_spawn_connection(server * srv,
					  plugin_data * p,
					  fcgi_extension_host * host, fcgi_proc * proc)
{
	...
	} else
	{
		...
		} else
		{
			...
			memcpy(&(fcgi_addr_in.sin_addr.s_addr), he->h_addr_list[0],
				   he->h_length);
	...
	if (-1 == connect(fcgi_fd, fcgi_addr, servlen))
	{
		...
		if (errno != ENOENT && !buffer_is_empty(proc->unixsocket))
		{
			unlink(proc->unixsocket->ptr);
		}
		...
		val = 1;
		if (setsockopt
			(fcgi_fd, SOL_SOCKET, SO_REUSEADDR, &val, sizeof(val)) < 0)
		{
			log_error_write(srv, __FILE__, __LINE__, "ss",
		...
		if (-1 == bind(fcgi_fd, fcgi_addr, servlen))
		{
			log_error_write(srv, __FILE__, __LINE__, "sbs",
							"bind failed for:",
							proc->connection_name, strerror(errno));
			return -1;
		}
		...
		case 0:
			{
				...
				if (host->bin_env_copy->used)
				{
					...
					{
						data_string *ds =
							(data_string *) host->bin_env_copy->data[i];
						char *ge;

						if (NULL != (ge = getenv(ds->value->ptr)))
						{
							env_add(&env,
									CONST_BUF_LEN(ds->value), ge, strlen(ge));
						}
					}
				} else
				{
				...
				for (i = 0; i < env.used; i++)
				{
					 ...
					 */
					if (0 ==
						strncmp(env.ptr[i], "PHP_FCGI_CHILDREN=",
				...
				/*
				 * chdir into the base of the bin-path, search for the last / 
				 */
				...
				/*
				 * log_error_write(srv, __FILE__, __LINE__, "sbs", "execve
				 * failed for:", host->bin_path, strerror(errno)); 
				 */
		...
		default:
			...
			default:
				log_error_write(srv, __FILE__, __LINE__, "sbs",
								"the fastcgi-backend", host->bin_path,
								"failed to start:");
				/*
...
SETDEFAULTS_FUNC(mod_fastcgi_set_defaults)
{
	...
	config_values_t cv[] = {
		{"fastcgi.server", NULL, T_CONFIG_LOCAL, T_CONFIG_SCOPE_CONNECTION}
		,						/* 0 */
		{"fastcgi.debug", NULL, T_CONFIG_SHORT, T_CONFIG_SCOPE_CONNECTION}
		,						/* 1 */
		{"fastcgi.map-extensions", NULL, T_CONFIG_ARRAY,
		...
		if (NULL != (du = array_get_element(ca, "fastcgi.server")))
		{
			...
			for (j = 0; j < da->value->used; j++)
			{
				...
				for (n = 0; n < da_ext->value->used; n++)
				{
					...
					config_values_t fcv[] = {
						{"host", NULL, T_CONFIG_STRING,
						 T_CONFIG_SCOPE_CONNECTION}
						,		/* 0 */
						{"docroot", NULL, T_CONFIG_STRING,
						 T_CONFIG_SCOPE_CONNECTION}
						,		/* 1 */
						{"mode", NULL, T_CONFIG_STRING,
						 T_CONFIG_SCOPE_CONNECTION}
						,		/* 2 */
						{"socket", NULL, T_CONFIG_STRING,
						 T_CONFIG_SCOPE_CONNECTION}
						,		/* 3 */
						{"bin-path", NULL, T_CONFIG_STRING,
						 T_CONFIG_SCOPE_CONNECTION}
						,		/* 4 */

						{"check-local", NULL, T_CONFIG_BOOLEAN,
						 T_CONFIG_SCOPE_CONNECTION}
						,		/* 5 */
						{"port", NULL, T_CONFIG_SHORT,
						 T_CONFIG_SCOPE_CONNECTION}
						,		/* 6 */
						{"min-procs-not-working", NULL,
						 T_CONFIG_SHORT,
						 T_CONFIG_SCOPE_CONNECTION}
						,		/* 7 this is broken for now */
						{"max-procs", NULL, T_CONFIG_SHORT,
						 T_CONFIG_SCOPE_CONNECTION}
						,		/* 8 */
						{"max-load-per-proc", NULL, T_CONFIG_SHORT,
						 T_CONFIG_SCOPE_CONNECTION}
						,		/* 9 */
						{"idle-timeout", NULL, T_CONFIG_SHORT,
						 T_CONFIG_SCOPE_CONNECTION}
						,		/* 10 */
						{"disable-time", NULL, T_CONFIG_SHORT,
						 T_CONFIG_SCOPE_CONNECTION}
						,		/* 11 */

						{"bin-environment", NULL, T_CONFIG_ARRAY,
						 T_CONFIG_SCOPE_CONNECTION}
						,		/* 12 */
						{"bin-copy-environment", NULL, T_CONFIG_ARRAY,
						 T_CONFIG_SCOPE_CONNECTION}
						,		/* 13 */

						{"broken-scriptfilename", NULL,
						 T_CONFIG_BOOLEAN,
						 T_CONFIG_SCOPE_CONNECTION}
						,		/* 14 */
						{"allow-x-send-file", NULL, T_CONFIG_BOOLEAN,
						 T_CONFIG_SCOPE_CONNECTION}
						,		/* 15 */
						{"strip-request-uri", NULL, T_CONFIG_STRING,
						 T_CONFIG_SCOPE_CONNECTION}
						,		/* 16 */
						{"kill-signal", NULL, T_CONFIG_SHORT,
						 T_CONFIG_SCOPE_CONNECTION}
						,		/* 17 */
					...
					if (!buffer_is_empty(host->bin_path))
					{
						...
						/*
						 * HACK: just to make sure the adaptive spawing is
						 * disabled 
						 */
						...
						for (pno = 0; pno < host->min_procs; pno++)
						{
							...
							} else
							{
								buffer_copy_string_buffer
									(proc->unixsocket, host->unixsocket);
								buffer_append_string_len(proc->
														 unixsocket,
														 CONST_STR_LEN("-"));
					...
					} else
					{
						...
						{
							buffer_copy_string_buffer(proc->
													  unixsocket,
													  host->unixsocket);
						}
...
static int
fcgi_set_state(server * srv, handler_ctx * hctx, fcgi_connection_state_t state)
{
	hctx->state = state;
...
void fcgi_connection_close(server * srv, handler_ctx * hctx)
{
	...
	if (hctx->host && hctx->proc)
	{
		if (hctx->got_proc)
		{
			...
			status_counter_set(srv, CONST_BUF_LEN(p->statuskey),
							   hctx->proc->load);
...
static int fcgi_reconnect(server * srv, handler_ctx * hctx)
{
	...
	/*
	 * child died 1. connect was ok, connection was accepted but the php
	 * accept loop checks after the accept if it should die or not. if yes we
	 * can only detect it at a write() next step is resetting this attemp and
	 * setup a connection again if we have more than 5 reconnects for the same 
	 * request, die 2. we have a connection but the child died by some other
	 * reason 
	 */
...
static handler_t
fcgi_connection_reset(server * srv, connection * con, void *p_d)
{
	plugin_data *p = p_d;
...
static int
fcgi_env_add(buffer * env, const char *key, size_t key_len,
			 const char *val, size_t val_len)
{
	 ...
	 */
	if (key_len > 0x7fffffff)
		key_len = 0x7fffffff;
	if (val_len > 0x7fffffff)
		val_len = 0x7fffffff;
...
static int
fcgi_header(FCGI_Header * header, unsigned char type, size_t request_id,
...
static connection_result_t
fcgi_establish_connection(server * srv, handler_ctx * hctx)
{
	struct sockaddr *fcgi_addr;
	struct sockaddr_in fcgi_addr_in;
#ifdef HAVE_SYS_UN_H
	...
	} else
	{
		...
		if (!buffer_is_empty(host->host))
		{
			...
			{
				log_error_write(srv, __FILE__, __LINE__, "sbs",
								"converting IP address failed for",
								host->host,
	...
	if (-1 == connect(fcgi_fd, fcgi_addr, servlen))
	{
		...
		} else if (errno == EAGAIN)
		{
			if (hctx->conf.debug)
			{
				log_error_write(srv, __FILE__, __LINE__, "sbsd",
								...
								"The load for this FastCGI backend",
		...
		} else
		{
			log_error_write(srv, __FILE__, __LINE__, "sssb",
							"connect failed:",
							strerror(errno), "on", proc->connection_name);
...
static int
fcgi_env_add_request_headers(server * srv, connection * con, plugin_data * p)
{
	size_t i;
	...
	for (i = 0; i < con->request.headers->used; i++)
	{
		...
		if (ds->value->used && ds->key->used)
		{
			...
			FCGI_ENV_ADD_CHECK(fcgi_env_add
							   (p->fcgi_env,
								CONST_BUF_LEN(srv->tmp_buf),
								CONST_BUF_LEN(ds->value)), con);
		}
	}
...
static int fcgi_create_env(server * srv, handler_ctx * hctx, size_t request_id)
{
	...
	fcgi_header(&(beginRecord.header), FCGI_BEGIN_REQUEST, request_id,
	...
	FCGI_ENV_ADD_CHECK(fcgi_env_add
					   (p->fcgi_env, CONST_STR_LEN("SERVER_SOFTWARE"),
						CONST_STR_LEN(PACKAGE_NAME "/" PACKAGE_VERSION)),
					   con) if (con->server_name->used)
	{
	FCGI_ENV_ADD_CHECK(fcgi_env_add
						   (p->fcgi_env, CONST_STR_LEN("SERVER_NAME"),
								CONST_BUF_LEN(con->server_name)), con)} else
	{
#ifdef HAVE_IPV6
	...
	FCGI_ENV_ADD_CHECK(fcgi_env_add
					   (p->fcgi_env, CONST_STR_LEN("GATEWAY_INTERFACE"),
						CONST_STR_LEN("CGI/1.1")), con) LI_ltostr(buf,
#ifdef HAVE_IPV6
																  ntohs
																  (srv_sock->
																   addr.
	...
	FCGI_ENV_ADD_CHECK(fcgi_env_add
					   (p->fcgi_env, CONST_STR_LEN("SERVER_PORT"), buf,
						strlen(buf)), con)
		/*
	...
	FCGI_ENV_ADD_CHECK(fcgi_env_add
					   (p->fcgi_env, CONST_STR_LEN("SERVER_ADDR"), s,
						strlen(s)), con) LI_ltostr(buf,
#ifdef HAVE_IPV6
												   ntohs(con->dst_addr.
														 plain.
														 sa_family ? con->
	...
	FCGI_ENV_ADD_CHECK(fcgi_env_add
					   (p->fcgi_env, CONST_STR_LEN("REMOTE_PORT"), buf,
						strlen(buf)), con) s =
		inet_ntop_cache_get_ip(srv, &(con->dst_addr));
	FCGI_ENV_ADD_CHECK(fcgi_env_add
					   (p->fcgi_env, CONST_STR_LEN("REMOTE_ADDR"), s,
						strlen(s)), con) if (!buffer_is_empty(con->authed_user))
	{
		/*
		 * AUTH_TYPE fix by Troy Kruthoff (tkruthoff@gmail.com) section 4.1.1
		 * of RFC 3875 (cgi spec) requires the server to set a AUTH_TYPE env
		 * declaring the type of authentication used.  (see
		 * http://tools.ietf.org/html/rfc3875#page-11) I copied this code from 
		 * mod_auth.c where it extracts auth info from the "Authorization"
		 * header to authenticate the user before allowing the request to
		 * proceed.  I'm guessing it makes sense to re-parse the header here,
		 * as mod_auth is unaware if the request is headed for cgi/fcgi.
		 * Someone more familiar with the lighty internals should be able to
		 * quickly determine if we are better storing AUTH_TYPE on the initial 
		 * parse in mod_auth. 
		 */
		...
		FCGI_ENV_ADD_CHECK(fcgi_env_add
						   (p->fcgi_env, CONST_STR_LEN("REMOTE_USER"),
							CONST_BUF_LEN(con->authed_user)),
						   con) if (NULL != (ds = (data_string *)
			...
			if (NULL != (auth_realm = strchr(http_authorization, ' ')))
			{
						...
						strncmp(http_authorization, "Basic", auth_type_len)))
				{
					fcgi_env_add(p->fcgi_env,
						   ...
						   && (0 ==
							   strncmp(http_authorization, "Digest",
									   auth_type_len)))
				{
					fcgi_env_add(p->fcgi_env,
								 CONST_STR_LEN("AUTH_TYPE"),
	...
	if (host->mode != FCGI_AUTHORIZER)
	{
		...
		FCGI_ENV_ADD_CHECK(fcgi_env_add
						   ...
						   con) if (!buffer_is_empty(con->request.pathinfo))
		{
			FCGI_ENV_ADD_CHECK(fcgi_env_add
							   (p->fcgi_env, CONST_STR_LEN("PATH_INFO"),
								CONST_BUF_LEN(con->request.pathinfo)), con)
				/*
	...
	FCGI_ENV_ADD_CHECK(fcgi_env_add
						   ...
						   con) FCGI_ENV_ADD_CHECK(fcgi_env_add(p->
																	fcgi_env,
																	CONST_STR_LEN
																	("DOCUMENT_ROOT"),
																	CONST_BUF_LEN
													   ...
													   con)} else
	{
		...
		/*
		 * cgi.fix_pathinfo need a broken SCRIPT_FILENAME to find out what
		 * PATH_INFO is itself see src/sapi/cgi_main.c, init_request_info() 
		 */
	...
	if (host->strip_request_uri->used > 1)
	{
		 ...
		 */
		if ('/' !=
			host->strip_request_uri->ptr[host->strip_request_uri->used - 2])
		{
			/*
			 * fix the user-input to have / as last char 
			 */
			buffer_append_string_len(host->strip_request_uri,
									 CONST_STR_LEN("/"));
		}

		if (con->request.orig_uri->used >= host->strip_request_uri->used &&
			0 == strncmp(con->request.orig_uri->ptr,
						 host->strip_request_uri->ptr,
						 host->strip_request_uri->used - 1))
		{
			/*
			...
			fcgi_env_add(p->fcgi_env, CONST_STR_LEN("REQUEST_URI"),
						 ...
						 (host->strip_request_uri->used - 2),
						 con->request.orig_uri->used -
						 (host->strip_request_uri->used - 2));
		} else
		{
		FCGI_ENV_ADD_CHECK(fcgi_env_add
							   (p->fcgi_env,
									...
									CONST_BUF_LEN(con->request.orig_uri)), con)}
	} else
	{
	FCGI_ENV_ADD_CHECK(fcgi_env_add
						   (p->fcgi_env, CONST_STR_LEN("REQUEST_URI"),
								CONST_BUF_LEN(con->request.orig_uri)), con)}
	...
	FCGI_ENV_ADD_CHECK(fcgi_env_add
						   (p->fcgi_env,
								...
								CONST_BUF_LEN(con->request.uri)), con)}
	if (!buffer_is_empty(con->uri.query))
	{
	FCGI_ENV_ADD_CHECK(fcgi_env_add
						   (p->fcgi_env,
								CONST_STR_LEN("QUERY_STRING"),
								CONST_BUF_LEN(con->uri.query)), con)} else
	{
	FCGI_ENV_ADD_CHECK(fcgi_env_add
						   (p->fcgi_env,
								CONST_STR_LEN("QUERY_STRING"),
	...
	FCGI_ENV_ADD_CHECK(fcgi_env_add(p->fcgi_env, CONST_STR_LEN("REQUEST_METHOD"), s, strlen(s)), con) FCGI_ENV_ADD_CHECK(fcgi_env_add(p->fcgi_env, CONST_STR_LEN("REDIRECT_STATUS"), CONST_STR_LEN("200")), con)	/* if 
																																																					 * php 
																																																					 * is 
																																																					 * compiled 
																																																					 * with 
	...
	FCGI_ENV_ADD_CHECK(fcgi_env_add
					   (p->fcgi_env, CONST_STR_LEN("SERVER_PROTOCOL"), s,
						strlen(s)), con) if (srv_sock->is_ssl
											 || srv_sock->is_proxy_ssl)
	{
	FCGI_ENV_ADD_CHECK(fcgi_env_add
						   (p->fcgi_env, CONST_STR_LEN("HTTPS"),
								CONST_STR_LEN("on")), con)}
	...
	if (con->request.content_length)
	{
		...
		{
			off_t weWant =
				req_cq->bytes_in - offset >
				FCGI_MAX_LENGTH ? FCGI_MAX_LENGTH : req_cq->bytes_in - offset;
			off_t written = 0;
			off_t weHave = 0;

			/*
			 * we announce toWrite octets now take all the request_content
			 * chunks that we need to fill this request 
			 */
			...
			if (p->conf.debug > 10)
			{
				log_error_write(srv, __FILE__, __LINE__, "soso",
								"tosend:", offset, "/", req_cq->bytes_in);
			}
			...
			for (written = 0; written != weWant;)
			{
				...
				case FILE_CHUNK:
					...
					chunkqueue_append_file(hctx->wb,
										   req_c->file.name,
										   req_c->offset, weHave);
					...
					/*
					 * steal the tempfile This is tricky: - we reference the
					 * tempfile from the request-content-queue several times if 
					 * the req_c is larger than FCGI_MAX_LENGTH - we can't
					 * simply cleanup the request-content-queue as soon as
					 * possible as it would remove the tempfiles - the idea is
					 * to 'steal' the tempfiles and attach the is_temp flag to
					 * the last referencing chunk of the fastcgi-write-queue 
					 */
	...
	for (i = 0; i < hctx->write_buffer->used; i++)
	{
		...
		if ((i + 1) % 16 == 0)
		{
			...
			{
				fprintf(stderr, "%c",
						isprint((unsigned char) hctx->write_buffer->
								ptr[j]) ? hctx->write_buffer->ptr[j] : '.');
			}
			fprintf(stderr, "\n");
		}
	}
...
static int
fcgi_response_parse(server * srv, connection * con, plugin_data * p,
					buffer * in)
{
	char *s, *ns;
	...
	for (s = p->parse_response->ptr; NULL != (ns = strchr(s, '\n')); s = ns + 1)
	{
		...
		if (ns > p->parse_response->ptr && *(ns - 1) == '\r')
		{
			*(ns - 1) = '\0';
		}
		...
		{
			/*
			 * authorizers shouldn't affect the response headers sent back to
			 * the client 
			 */
			...
			if (0 != strncasecmp(key, "Status", key_len))
			{
				if (NULL ==
					(ds =
					 (data_string *) array_get_unused_element(con->
															  response.
															  headers,
															  TYPE_STRING)))
				{
...
static int
fastcgi_get_packet(server * srv, handler_ctx * hctx,
				   fastcgi_response_packet * packet)
{
	...
	for (c = hctx->rb->first; c; c = c->next)
	{
		if (packet->b->used == 0)
		{
			buffer_copy_string_len(packet->b, c->mem->ptr + c->offset,
								   c->mem->used - c->offset - 1);
		} else
		{
			buffer_append_string_len(packet->b, c->mem->ptr + c->offset,
									 c->mem->used - c->offset - 1);
		}

		if (packet->b->used >= sizeof(*header) + 1)
			break;
	}

	if ((packet->b->used == 0) || (packet->b->used - 1 < sizeof(FCGI_Header)))
	{
		/*
		 * no header 
		 */
		...
		log_error_write(srv, __FILE__, __LINE__, "sdsds",
						"FastCGI: header too small:", packet->b->used,
						"bytes <", sizeof(FCGI_Header), "bytes");
		return -1;
	}
	...
	packet->len =
		(header->contentLengthB0 | (header->contentLengthB1 << 8)) +
	...
	if (packet->len)
	{
		...
		for (; c && (packet->b->used < packet->len + 1); c = c->next)
		{
			...
			buffer_append_string_len(packet->b,
									 c->mem->ptr + c->offset + offset, weHave);
...
static int fcgi_demux_response(server * srv, handler_ctx * hctx)
{
	...
	if (toread > 0)
	{
		...
		if (-1 == (r = read(hctx->fd, b->ptr, toread)))
		{
			...
			{
				/*
				 * roll back the last chunk allocation, and continue on next
				 * iteration 
				 */
	...
	while (fin == 0)
	{
		...
		case FCGI_STDOUT:
			...
			if (0 == con->file_started)
			{
				...
				/*
				 * search for header terminator if we start with \r\n check if 
				 * last packet terminated with \r\n if we start with \n check
				 * if last packet terminated with \n search for \r\n\r\n search 
				 * for \n\n 
				 */
				...
				{
					buffer_append_string_buffer(hctx->
												response_header, packet.b);
				}

				if (NULL !=
					(c =
					 buffer_search_string_len(hctx->response_header,
											  CONST_STR_LEN("\r\n\r\n"))))
				{
					blen =
						hctx->response_header->used - (c -
													   hctx->
													   response_header->
													   ptr) - 4;
					hctx->response_header->used =
						(c - hctx->response_header->ptr) + 3;
				...
				{
					/*
					 * a authorizer with approved the static request, ignore
					 * the content here 
					 */
				...
				if (host->allow_xsendfile &&
					(NULL !=
					 ...
					 || NULL != (ds = (data_string *)
								 array_get_element(con->response.
												   headers, "X-Sendfile"))))
				{
					stat_cache_entry *sce;

					if (HANDLER_ERROR !=
						stat_cache_get_entry(srv, con, ds->value, &sce))
					{
						data_string *dcls;
						if (NULL == (dcls = (data_string *)
						 ...
						 * found 
						 */
						http_chunk_append_file(srv, con, ds->value,
											   0, sce->st.st_size);
						hctx->send_content_body = 0;	/* ignore the content */
						joblist_append(srv, con);

						buffer_copy_string_len(dcls->key,
											   "Content-Length",
											   sizeof("Content-Length") - 1);
						buffer_copy_long(dcls->value, sce->st.st_size);
						dcls =
							(data_string *) array_replace(con->
														  response.
														  headers,
														  (data_unset *) dcls);
						if (dcls)
							dcls->free((data_unset *) dcls);
...
static int
fcgi_restart_dead_procs(server * srv, plugin_data * p,
						fcgi_extension_host * host)
{
	fcgi_proc *proc;
	...
	for (proc = host->first; proc; proc = proc->next)
	{
		...
		case PROC_STATE_DIED:
			/*
			 * local procs get restarted by us, remote ones hopefully by the
			 * admin 
			 */
			...
			{
				/*
				 * we still have connections bound to this proc, let them
				 * terminate first 
				 */
				if (proc->load != 0)
					break;
...
static handler_t fcgi_write_request(server * srv, handler_ctx * hctx)
{
	...
	/*
	 * sanity check: - host != NULL - either: - tcp socket (do not check
	 * host->host->uses, as it may be not set which means INADDR_LOOPBACK) -
	 * unix socket 
	 */
	if (!host || (!host->port && !host->unixsocket->used))
	{
	...
	if (hctx->state == FCGI_STATE_CONNECT_DELAYED)
	{
		 ...
		 */
		if (0 !=
			getsockopt(hctx->fd, SOL_SOCKET, SO_ERROR, &socket_error,
					   &socket_error_len))
		{
			log_error_write(srv, __FILE__, __LINE__, "ss",
	...
	case FCGI_STATE_INIT:
		...
		for (proc = hctx->host->first;
			 proc && proc->state != PROC_STATE_RUNNING; proc = proc->next);
		...
		case CONNECTION_DELAYED:
			/*
			 * connection is in progress, wait for an event and call
			 * getsockopt() below 
			 */
		...
		case CONNECTION_OVERLOADED:
			/*
			 * cool down the backend, it is overloaded -> EAGAIN 
			 */
			...
			fastcgi_status_copy_procname(p->statuskey, hctx->host, hctx->proc);
			buffer_append_string_len(p->statuskey,
		...
		case CONNECTION_DEAD:
			/*
			 * we got a hard error from the backend like - ECONNREFUSED for
			 * tcp-ip sockets - ENOENT for unix-domain-sockets for check if
			 * the host is back in 5 seconds 
			 */
	...
	case FCGI_STATE_PREPARE_WRITE:
		...
		{
			log_error_write(srv, __FILE__, __LINE__, "sd",
							"fcgi-request is already in use:",
							hctx->request_id);
		}
		 ...
		 * fall through 
		 */
		if (-1 == fcgi_create_env(srv, hctx, hctx->request_id))
			return HANDLER_ERROR;
	...
	case FCGI_STATE_WRITE:
		...
		if (ret < 0)
		{
			...
			case ENOTCONN:
				/*
				 * the connection got dropped after accept() this is most of
				 * the time a PHP which dies after PHP_FCGI_MAX_REQUESTS 
				 */
				if (hctx->wb->bytes_out == 0 && hctx->reconnects < 5)
				{
					usleep(10000);	/* take away the load of the webserver to
									 * give the php a chance to restart */
				...
				/*
				 * not reconnected ... why far@#lighttpd report this for
				 * FreeBSD 
				 */
...
SUBREQUEST_FUNC(mod_fastcgi_handle_subrequest)
{
	...
	/*
	 * we don't have a host yet, choose one -> this happens in the first round
	...
	if (hctx->host == NULL)
	{
		...
		if (ndx >= (int) hctx->ext->used || ndx < 0)
			ndx = 0;
		...
		/*
		 * we put a connection on this host, move the other new connections to
		 * other hosts as soon as hctx->host is unassigned, decrease the load
		 * again 
		 */
		hctx->host->load++;
		hctx->proc = NULL;
	...
	case HANDLER_ERROR:
			...
			hctx->state == FCGI_STATE_CONNECT_DELAYED)
		{
			...
			/*
			 * cleanup this request and let the request handler start this
			 * request again 
			 */
			if (hctx->reconnects < 5)
			{
				fcgi_reconnect(srv, hctx);
				joblist_append(srv, con);	/* in case we come from the
											 * event-handler */
...
static handler_t fcgi_handle_fdevent(void *s, void *ctx, int revents)
{
	...
	if ((revents & FDEVENT_IN) && hctx->state == FCGI_STATE_READ)
	{
		...
		case 1:
			...
			if (host->mode == FCGI_AUTHORIZER &&
				(con->http_status == 200 || con->http_status == 0))
			{
				/*
				...
				con->mode = DIRECT;
				con->file_started = 1;	/* fcgi_extension won't touch the
										 * request afterwards */
			} else
			{
				/*
		...
		case -1:
			...
			if (con->file_started == 0)
			{
				...
				log_error_write(srv, __FILE__, __LINE__, "sosbsbs",
								...
								proc->connection_name, "for",
								con->uri.path, ", closing connection");
			...
			{
				/*
				 * response might have been already started, kill the
				 * connection 
				 */
				fcgi_connection_close(srv, hctx);
	...
	if (revents & FDEVENT_OUT)
	{
		...
		{
			/*
			 * we are allowed to send something out 1. in an unfinished
			 * connect() call 2. in an unfinished write() call (long POST
			 * request) 
			 */
	...
	if (revents & FDEVENT_HUP)
	{
		...
		{
			/*
			 * getoptsock will catch this one (right ?) if we are in connect
			 * we might get an EINPROGRESS in the first call and an FDEVENT_HUP 
			 * in the second round FIXME: as it is a bit ugly. 
			 */
			return mod_fastcgi_handle_subrequest(srv, con, p);
		} else if (hctx->state == FCGI_STATE_READ && hctx->proc->port == 0)
		{
			/*
			 * FIXME: ioctl says 8192 bytes to read from PHP and we receive
			 * directly a HUP for the socket even if the FCGI_FIN packet is not 
			 * received yet 
			 */
		} else
		{
...
#define PATCH(x) \
	p->conf.x = s->x;
static int
fcgi_patch_connection(server * srv, connection * con, plugin_data * p)
{
	size_t i, j;
	...
	for (i = 1; i < srv->config_context->used; i++)
	{
		...
		for (j = 0; j < dc->value->used; j++)
		{
			...
			if (buffer_is_equal_string
				(du->key, CONST_STR_LEN("fastcgi.server")))
			{
				PATCH(exts);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("fastcgi.debug")))
			{
				PATCH(debug);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("fastcgi.map-extensions")))
			{
				PATCH(ext_mapping);
			}
		}
...
static handler_t
fcgi_check_extension(server * srv, connection * con, void *p_d,
					 int uri_path_handler)
{
	...
	/*
	 * fastcgi.map-extensions maps extensions to existing fastcgi.server
	 * entries fastcgi.map-extensions = ( ".php3" => ".php" ) fastcgi.server
	 * = ( ".php" => ... ) 
	 */
		...
		for (k = 0; k < p->conf.exts->used; k++)
		{
			...
			if (extension->key->ptr[0] == '/')
			{
				if ((ct_len <= con->uri.path->used - 1) &&
					(strncmp
					 (con->uri.path->ptr, extension->key->ptr, ct_len) == 0))
					break;
			} else if ((ct_len <= s_len)
					   && (0 ==
						   strncmp(fn->ptr + s_len - ct_len,
	...
	if (uri_path_handler)
	{
		if (host->check_local == 0)
		{
			...
			/*
			 * the prefix is the SCRIPT_NAME, everything from start to the next 
			 * slash this is important for check-local = "disable" if prefix = 
			 * /admin.fcgi /admin.fcgi/foo/bar SCRIPT_NAME = /admin.fcgi
			 * PATH_INFO = /foo/bar if prefix = /fcgi-bin/ /fcgi-bin/foo/bar
			 * SCRIPT_NAME = /fcgi-bin/foo PATH_INFO = /bar if prefix = /, and
			 * fix-root-path-name is enable /fcgi-bin/foo/bar SCRIPT_NAME =
			 * /fcgi-bin/foo PATH_INFO = /bar 
			 */
 ...
 */
static handler_t
fcgi_check_extension_1(server * srv, connection * con, void *p_d)
{
 ...
 */
static handler_t
fcgi_check_extension_2(server * srv, connection * con, void *p_d)
{
...
JOBLIST_FUNC(mod_fastcgi_handle_joblist)
{
	...
	if (hctx->fd != -1)
	{
		...
		default:
			log_error_write(srv, __FILE__, __LINE__, "sd",
							"unhandled fcgi.state", hctx->state);
			break;
		}
	}
...
static handler_t
fcgi_connection_close_callback(server * srv, connection * con, void *p_d)
{
	plugin_data *p = p_d;
...
TRIGGER_FUNC(mod_fastcgi_handle_trigger)
{
	...
	/*
	 * perhaps we should kill a connect attempt after 10-15 seconds currently
	 * we wait for the TCP timeout which is 180 seconds on Linux 
	 */
		...
		for (j = 0; j < exts->used; j++)
		{
			...
			for (n = 0; n < ex->used; n++)
			{
					...
					(sum_load / host->num_procs) > host->max_load_per_proc)
				{
					...
					for (proc = host->unused_procs;
						 proc && proc->pid != 0; proc = proc->next);
				...
				for (proc = host->first; proc; proc = proc->next)
				{
					if (proc->load != 0)
						break;
					if (host->num_procs <= host->min_procs)
						break;
					if (proc->pid == 0)
						continue;
					...
					{
						/*
						 * a proc is idling for a long time now, terminate it 
						 */
						...
						/*
						 * proc is now in unused, let the next second handle
						 * the next process 
						 */
						break;
					}
