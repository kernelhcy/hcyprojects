...
enum { FDEVENT_HANDLED_UNSET, FDEVENT_HANDLED_FINISHED,
	FDEVENT_HANDLED_NOT_FINISHED, FDEVENT_HANDLED_ERROR
};
...
SETDEFAULTS_FUNC(mod_fastcgi_set_defaults)
{
	...
	config_values_t cv[] = {
		{"cgi.assign", NULL, T_CONFIG_ARRAY, T_CONFIG_SCOPE_CONNECTION}
		,						/* 0 */
...
static int
cgi_response_parse(server * srv, connection * con, plugin_data * p, buffer * in)
{
	char *ns;
	const char *s;
	int line = 0;
...
static int cgi_demux_response(server * srv, handler_ctx * hctx)
{
	...
	while (1)
	{
		...
		if (con->file_started == 0)
		{
			...
			for (i = 0;
				 !is_header_end && i < hctx->response_header->used - 1; i++)
			{
				...
				case ':':
					/*
					 * we found a colon looks like we have a normal header 
					 */
					is_header = 1;
					break;
				case '\n':
...
static handler_t cgi_connection_close(server * srv, handler_ctx * hctx)
{
	 ...
	 * to the CGI script close cgi-connection 
	 */
	...
	if (hctx->fd != -1)
	{
		...
		if (close(hctx->fd))
		{
			log_error_write(srv, __FILE__, __LINE__, "sds",
							"cgi close failed ", hctx->fd, strerror(errno));
		}
	...
	if (pid)
	{
		...
		case 0:
			...
			log_error_write(srv, __FILE__, __LINE__, "sd",
							"(debug) child isn't done yet, pid:", pid);
#endif
			break;
		case -1:
			/*
...
static handler_t
cgi_connection_close_callback(server * srv, connection * con, void *p_d)
{
	plugin_data *p = p_d;
...
static handler_t cgi_handle_fdevent(void *s, void *ctx, int revents)
{
	...
	if (revents & FDEVENT_IN)
	{
		...
		case FDEVENT_HANDLED_FINISHED:
			...
			log_error_write(srv, __FILE__, __LINE__, "ddss", con->fd,
							hctx->fd, connection_get_state(con->state),
							"finished");
#endif
			cgi_connection_close(srv, hctx);

			/*
			 * if we get a IN|HUP and have read everything don't exec the close 
			 * twice 
			 */
	...
	{
		/*
		 * check if we still have a unfinished header package which is a body
		 * in reality 
		 */
...
# if 0
		log_error_write(srv, __FILE__, __LINE__, "sddd",
						"got HUP from cgi", con->fd, hctx->fd, revents);
# endif
...
static int
cgi_env_add(char_array * env, const char *key, size_t key_len,
...
static int
cgi_create_env(server * srv, connection * con, plugin_data * p,
			   buffer * cgi_handler)
{
	pid_t pid;
	...
	if (pipe(to_cgi_fds))
	{
		log_error_write(srv, __FILE__, __LINE__, "ss", "pipe failed:",
						strerror(errno));
		return -1;
	}
	...
	case 0:
		{
			...
			/*
			 * HACK: this is not nice, but it works we feed the stderr of the
			 * CGI to our errorlog, if possible 
			 */
			...
			LI_ltostr(buf,
#ifdef HAVE_IPV6
					  ntohs(con->dst_addr.plain.sa_family ==
							AF_INET6 ? con->dst_addr.ipv6.
			...
			{
				cgi_env_add(&env, CONST_STR_LEN("LD_LIBRARY_PATH"), s,
							strlen(s));
			}
#ifdef __CYGWIN__
			/*
			...
			/*
			 * log_error_write(srv, __FILE__, __LINE__, "sss", "CGI failed:",
				...
				for (c = cq->first; c; c = cq->first)
				{
					...
					case FILE_CHUNK:
						...
						if (c->file.mmap.start == MAP_FAILED)
						{
							...
							/*
							 * chunk_reset() or chunk_free() will cleanup for
							 * us 
							 */
						}
...
#define PATCH(x) \
	p->conf.x = s->x;
static int
mod_cgi_patch_connection(server * srv, connection * con, plugin_data * p)
{
	size_t i, j;
...
TRIGGER_FUNC(cgi_trigger)
{
	...
	for (ndx = 0; ndx < p->cgi_pid.used; ndx++)
	{
...
#if 0
			log_error_write(srv, __FILE__, __LINE__, "sd",
							"(debug) child isn't done yet, pid:",
							p->cgi_pid.ptr[ndx]);
#endif
			break;
		case -1:
		...
		default:
...
#if 0
				log_error_write(srv, __FILE__, __LINE__, "sd",
								"(debug) cgi exited fine, pid:",
								p->cgi_pid.ptr[ndx]);
#endif
			} else if (WIFSIGNALED(status))
			{
				/*
				 * FIXME: what if we killed the CGI script with a kill(...,
				 * SIGTERM) ? 
				 */
				if (WTERMSIG(status) != SIGTERM)
				{
					log_error_write(srv, __FILE__, __LINE__, "sd",
									"cleaning up CGI: process died with signal",
									WTERMSIG(status));
				}
			} else
			{
			...
			/*
			 * del modified the buffer structure and copies the last entry to
			 * the current one -> recheck the current index 
			 */
			ndx--;
		}
	}
...
SUBREQUEST_FUNC(mod_cgi_handle_subrequest)
{
...
#if 0
	log_error_write(srv, __FILE__, __LINE__, "sdd", "subrequest, pid =",
					hctx, hctx->pid);
#endif
	if (hctx->pid == 0)
		return HANDLER_FINISHED;
	...
	case -1:
		...
		log_error_write(srv, __FILE__, __LINE__, "ss", "waitpid failed: ",
						strerror(errno));
		con->mode = DIRECT;
		con->http_status = 500;
	...
	default:
		/*
		 * cgi process exited cleanly check if we already got the response 
		 */
