							  ...
							  const config_values_t cv[])
{
	...
	for (i = 0; cv[i].key; i++)
	{
			...
			if (du->type == TYPE_ARRAY)
			{
				...
				for (j = 0; j < da->value->used; j++)
				{
					if (da->value->data[j]->type == TYPE_STRING)
					{
						...
						{
							/*
							 * the id's were generated automaticly, as we copy
							 * now we might have to renumber them this is used
							 * to prepend server.modules by mod_indexfiles as
							 * it has to be loaded before mod_fastcgi and
							 * friends 
							 */
						...
						array_insert_unique(cv[i].destination,
											(data_unset *) ds);
					} else
					{
			...
			case TYPE_STRING:
				{
					...
					log_error_write(srv, __FILE__, __LINE__, "ssb",
									"got a string but expected a short:",
									cv[i].key, ds->value);
		...
		case T_CONFIG_BOOLEAN:
			if (du->type == TYPE_STRING)
			{
				...
				if (buffer_is_equal_string(ds->value, CONST_STR_LEN("enable")))
				{
					*((unsigned short *) (cv[i].destination)) = 1;
						   ...
						   (ds->value, CONST_STR_LEN("disable")))
				{
					*((unsigned short *) (cv[i].destination)) = 0;
				} else
				{
					log_error_write(srv, __FILE__, __LINE__, "ssbs",
									"ERROR: unexpected value for key:",
									cv[i].key, ds->value, "(enable|disable)");
			...
			{
				log_error_write(srv, __FILE__, __LINE__, "ssss",
								"ERROR: unexpected type for key:",
								cv[i].key, "(string)", "\"(enable|disable)\"");
		...
		case T_CONFIG_UNSUPPORTED:
			log_error_write(srv, __FILE__, __LINE__, "ssss",
							"ERROR: found unsupported key:", cv[i].key,
							"-", (char *) (cv[i].destination));
		...
		case T_CONFIG_DEPRECATED:
			log_error_write(srv, __FILE__, __LINE__, "ssss",
							"ERROR: found deprecated key:", cv[i].key,
							"-", (char *) (cv[i].destination));
...
{
#ifdef HAVE_IPV6
	return ntohs(addr->plain.sa_family ? addr->ipv6.sin6_port : addr->ipv4.
				 sin_port);
#else
...
static cond_result_t config_check_cond_cached(server * srv,
											  connection * con,
											  data_config * dc);

static cond_result_t
config_check_cond_nocache(server * srv, connection * con, data_config * dc)
{
	buffer *l;
			...
			if (!buffer_is_empty(con->uri.authority))
			{
				...
				case CONFIG_COND_EQ:
					...
					if (ck_colon)
					{
						...
						buffer_append_string_len(srv->cond_check_buf,
												 CONST_STR_LEN(":"));
					...
					} else if (!ck_colon)
					{
						 ...
						 */
						buffer_copy_string_len(srv->cond_check_buf,
											   l->ptr, val_colon - l->ptr);
						l = srv->cond_check_buf;
					}
					break;
				default:
	...
	case COMP_HTTP_REMOTE_IP:
		{
			char *nm_slash;
			/*
			 * handle remoteip limitations "10.0.0.1" is provided for all
			 * comparisions only for == and != we support "10.0.0.1/24" 
			 */
				...
				(NULL != (nm_slash = strchr(dc->string->ptr, '/'))))
			{
				...
				if (*err)
				{
					log_error_write(srv, __FILE__, __LINE__, "sbs",
									"ERROR: non-digit found in netmask:",
									dc->string, err);
...
#ifdef __WIN32
				if (INADDR_NONE ==
					(val_inp.s_addr = inet_addr(srv->cond_check_buf->ptr)))
				{
					log_error_write(srv, __FILE__, __LINE__, "sb",
									"ERROR: ip addr is invalid:",
									srv->cond_check_buf);
				...
				if ((val_inp.s_addr & nm) ==
					(con->dst_addr.ipv4.sin_addr.s_addr & nm))
				{
					return (dc->cond ==
							CONFIG_COND_EQ) ? COND_RESULT_TRUE :
						COND_RESULT_FALSE;
				} else
				{
					return (dc->cond ==
							CONFIG_COND_EQ) ? COND_RESULT_FALSE :
						COND_RESULT_TRUE;
				}
			} else
			{
	...
	if (NULL == l)
	{
		if (con->conf.log_condition_handling)
		{
			log_error_write(srv, __FILE__, __LINE__, "bsbs",
							dc->comp_key, "(", l, ") compare to NULL");
		}
		return COND_RESULT_FALSE;
	}
	...
	case CONFIG_COND_EQ:
		...
		{
			return (dc->cond ==
					CONFIG_COND_EQ) ? COND_RESULT_TRUE : COND_RESULT_FALSE;
		} else
		{
			return (dc->cond ==
					CONFIG_COND_EQ) ? COND_RESULT_FALSE : COND_RESULT_TRUE;
		}
		break;
#ifdef HAVE_PCRE_H
...
#define elementsof(x) (sizeof(x) / sizeof(x[0]))
#endif
			n = pcre_exec(dc->regex, dc->regex_study, l->ptr, l->used - 1,
						  0, 0, cache->matches, elementsof(cache->matches));
			...
			if (n > 0)
			{
				...
				return (dc->cond ==
						CONFIG_COND_MATCH) ? COND_RESULT_TRUE :
					COND_RESULT_FALSE;
			} else
			{
				/*
				 ...
				 */
				return (dc->cond ==
						CONFIG_COND_MATCH) ? COND_RESULT_FALSE :
					COND_RESULT_TRUE;
			}
			break;
		}
...
static cond_result_t
config_check_cond_cached(server * srv, connection * con, data_config * dc)
{
	...
	if (COND_RESULT_UNSET == caches[dc->context_ndx].result)
	{
		...
		{
			log_error_write(srv, __FILE__, __LINE__, "dss",
							dc->context_ndx, "(uncached) result:",
							caches[dc->context_ndx].result ==
...
void config_cond_cache_reset_item(server * srv, connection * con, comp_key_t item)
{
	...
	{
		if (item == COMP_LAST_ELEMENT || con->cond_cache[i].comp_type == item)
		{
...
int
config_append_cond_match_buffer(connection * con, data_config * dc,
								buffer * buf, int n)
{
	cond_cache_t *cache = &con->cond_cache[dc->context_ndx];
