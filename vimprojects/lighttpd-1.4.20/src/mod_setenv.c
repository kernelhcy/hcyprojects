...
typedef struct {
	int handled;				/* make sure that we only apply the headers
								 * once */
} handler_ctx;
...
SETDEFAULTS_FUNC(mod_setenv_set_defaults)
{
	...
	config_values_t cv[] = {
		{"setenv.add-request-header", NULL, T_CONFIG_ARRAY,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 0 */
		{"setenv.add-response-header", NULL, T_CONFIG_ARRAY,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 1 */
		{"setenv.add-environment", NULL, T_CONFIG_ARRAY,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 2 */
...
#define PATCH(x) \
	p->conf.x = s->x;
static int
mod_setenv_patch_connection(server * srv, connection * con, plugin_data * p)
{
	size_t i, j;
	...
	for (i = 1; i < srv->config_context->used; i++)
	{
		...
		for (j = 0; j < dc->value->used; j++)
		{
			...
			if (buffer_is_equal_string
				(du->key, CONST_STR_LEN("setenv.add-request-header")))
			{
				PATCH(request_header);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("setenv.add-response-header")))
			{
				PATCH(response_header);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("setenv.add-environment")))
			{
				PATCH(environment);
			}
		}
...
URIHANDLER_FUNC(mod_setenv_uri_handler)
{
	...
	for (k = 0; k < p->conf.environment->used; k++)
	{
		...
		if (NULL ==
			(ds_dst =
			 (data_string *) array_get_unused_element(con->environment,
													  TYPE_STRING)))
		{
			ds_dst = data_string_init();
		}
	...
	for (k = 0; k < p->conf.response_header->used; k++)
	{
		...
		response_header_insert(srv, con, CONST_BUF_LEN(ds->key),
