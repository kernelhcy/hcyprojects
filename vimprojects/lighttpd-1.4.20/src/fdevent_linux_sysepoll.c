...
static int
fdevent_linux_sysepoll_event_add(fdevents * ev, int fde_ndx, int fd, int events)
{
	struct epoll_event ep;
	int add = 0;
	...
	if (0 !=
		epoll_ctl(ev->epoll_fd, add ? EPOLL_CTL_ADD : EPOLL_CTL_MOD, fd, &ep))
	{
		fprintf(stderr, "%s.%d: epoll_ctl failed: %s, dying\n", __FILE__,
				__LINE__, strerror(errno));
...
#define SET(x) \
	...
	{
		fprintf(stderr,
				"%s.%d: epoll_create failed (%s), try to set server.event-handler = \"poll\" or \"select\"\n",
...
int fdevent_linux_sysepoll_init(fdevents * ev)
{
	...
	fprintf(stderr,
			"%s.%d: linux-sysepoll not supported, try to set server.event-handler = \"poll\" or \"select\"\n",
			__FILE__, __LINE__);
