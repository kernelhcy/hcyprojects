...
#  if 0
		if (ev->siginfo.si_band == POLLERR)
		{
			fprintf(stderr, "event: %d %02lx %02x %s\n",
					ev->siginfo.si_fd, ev->siginfo.si_band, errno,
					strerror(errno));
		}
#  endif
		if (ndx != 0)
...
static int fdevent_linux_rtsig_fcntl_set(fdevents * ev, int fd)
{
	...
	if (-1 == fcntl(fd, F_SETSIG, ev->signum))
		return -1;

	if (-1 == fcntl(fd, F_SETOWN, (int) pid))
		return -1;
...
int fdevent_linux_rtsig_init(fdevents * ev)
{
	...
	fprintf(stderr,
			"%s.%d: linux-rtsig not supported, try to set server.event-handler = \"poll\" or \"select\"\n",
			__FILE__, __LINE__);
	return -1;
}
#endif
