...
SETDEFAULTS_FUNC(mod_magnet_set_defaults)
{
	...
	config_values_t cv[] = {
		{MAGNET_CONFIG_RAW_URL, NULL, T_CONFIG_ARRAY,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 0 */
		{MAGNET_CONFIG_PHYSICAL_PATH, NULL, T_CONFIG_ARRAY,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 1 */
...
#define PATCH(x) \
	p->conf.x = s->x;
static int
mod_magnet_patch_connection(server * srv, connection * con, plugin_data * p)
{
	size_t i, j;
	...
	for (i = 1; i < srv->config_context->used; i++)
	{
		...
		for (j = 0; j < dc->value->used; j++)
		{
			...
			if (buffer_is_equal_string
				(du->key, CONST_STR_LEN(MAGNET_CONFIG_RAW_URL)))
			{
				PATCH(url_raw);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN(MAGNET_CONFIG_PHYSICAL_PATH)))
			{
				PATCH(physical_path);
			}
		}
...
static int magnet_print(lua_State * L)
{
	...
	log_error_write(srv, __FILE__, __LINE__, "ss", "(lua-print)", s);
...
static int magnet_atpanic(lua_State * L)
{
	...
	log_error_write(srv, __FILE__, __LINE__, "ss", "(lua-atpanic)", s);
...
static int magnet_reqhdr_get(lua_State * L)
{
	...
	if (NULL !=
		(ds = (data_string *) array_get_element(con->request.headers, key)))
	{
		if (ds->value->used)
		{
...
static buffer *magnet_env_get_buffer(server * srv, connection * con,
									 const char *key)
{
	buffer *dest = NULL;
	size_t i;
	...
	{
		if (0 == strcmp(key, env[i].name))
			break;
	}

	switch (env[i].type)
	{
	case MAGNET_ENV_PHYICAL_PATH:
		dest = con->physical.path;
		break;
	case MAGNET_ENV_PHYICAL_REL_PATH:
		dest = con->physical.rel_path;
		break;
	case MAGNET_ENV_PHYICAL_DOC_ROOT:
		dest = con->physical.doc_root;
		break;
	...
	case MAGNET_ENV_REQUEST_METHOD:
		...
		dest = srv->tmp_buf;
		break;
	case MAGNET_ENV_REQUEST_URI:
		dest = con->request.uri;
		break;
	case MAGNET_ENV_REQUEST_ORIG_URI:
		dest = con->request.orig_uri;
		break;
...
static int
magnet_copy_response_header(server * srv, connection * con,
							plugin_data * p, lua_State * L)
{
	UNUSED(p);
	/**
...
static handler_t
magnet_attract_array(server * srv, connection * con, plugin_data * p,
					 array * files)
{
	size_t i;
