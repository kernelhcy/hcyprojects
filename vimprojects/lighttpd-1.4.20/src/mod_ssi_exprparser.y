% token_prefix TK_ % token_type
{
buffer *}
%extra_argument {
ssi_ctx_t *ctx}
%name ssiexprparser % include {
#include <assert.h>
#include <string.h>
#ifdef HAVE_CONFIG_H
#include "config.h"
...
ssi_val_t *}
%type cond {
int} %token_destructor {
	buffer_free($$);
} %left AND. % left OR. % nonassoc EQ NE GT GE LT LE. % right NOT.
	input:: = exprline(B).
{
...
exprline(A):: = expr(B) cond(C) expr(D).
{
	...
	if (B->type == SSI_TYPE_STRING && D->type == SSI_TYPE_STRING)
	  {
