...
SETDEFAULTS_FUNC(mod_usertrack_set_defaults)
{
	...
	config_values_t cv[] = {
		{"usertrack.cookie-name", NULL, T_CONFIG_STRING,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 0 */
		{"usertrack.cookie-max-age", NULL, T_CONFIG_SHORT,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 1 */
		{"usertrack.cookie-domain", NULL, T_CONFIG_STRING,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 2 */

		{"usertrack.cookiename", NULL, T_CONFIG_DEPRECATED,
		 T_CONFIG_SCOPE_CONNECTION}
		,
...
#define PATCH(x) \
	p->conf.x = s->x;
static int
mod_usertrack_patch_connection(server * srv, connection * con, plugin_data * p)
{
	size_t i, j;
	...
	for (i = 1; i < srv->config_context->used; i++)
	{
		...
		for (j = 0; j < dc->value->used; j++)
		{
			...
			if (buffer_is_equal_string
				(du->key, CONST_STR_LEN("usertrack.cookie-name")))
			{
				PATCH(cookie_name);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("usertrack.cookie-max-age")))
			{
				PATCH(cookie_max_age);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("usertrack.cookie-domain")))
			{
				PATCH(cookie_domain);
			}
		}
...
URIHANDLER_FUNC(mod_usertrack_uri_handler)
{
	...
	if (NULL !=
		(ds =
		 (data_string *) array_get_element(con->request.headers, "Cookie")))
	{
		char *g;
		/*
		...
		/*
		 * parse the cookie check for cookiename + (WS | '=') 
		 */
		...
		if (NULL != (g = strstr(ds->value->ptr, p->conf.cookie_name->ptr)))
		{
			 ...
			 * skip WS 
			 */
			for (nc = g + p->conf.cookie_name->used - 1;
				 *nc == ' ' || *nc == '\t'; nc++);
	 ...
	 */
	MD5_Init(&Md5Ctx);
	MD5_Update(&Md5Ctx, (unsigned char *) con->uri.path->ptr,
			   con->uri.path->used - 1);
	...
	if (!buffer_is_empty(p->conf.cookie_domain))
	{
		...
		buffer_append_string_encoded(ds->value,
									 CONST_BUF_LEN(p->conf.cookie_domain),
									 ENCODING_REL_URI);
	}
