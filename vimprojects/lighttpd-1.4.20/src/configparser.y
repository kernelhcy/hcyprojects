% token_prefix TK_ % extra_argument
{
config_t *ctx}
%name configparser % include {
#include <assert.h>
#include <stdio.h>
#include <string.h>
#ifdef HAVE_CONFIG_H
...
#include "array.h"
	...
	/*
	 * op1 is to be eat/return by this function if success, op1->key is not
	 * cared op2 is left untouch, unreferenced 
	 */
	data_unset *configparser_merge_data(data_unset * op1,
										const data_unset * op2) {
		/*
		 * type mismatch 
		 */
			  ...
			  if (op1->type == TYPE_STRING && op2->type == TYPE_INTEGER)
				{
					...
					buffer_append_long(ds->value,
									   ((data_integer *) op2)->value);
					return op1;
...
input:: = metalines.
	...
	metalines:: =.metaline:: = varline.metaline:: = global.
	metaline:: = condlines(A) EOL.{ A = NULL;
}

metaline:: = include.metaline:: = include_shell.metaline:: = EOL. % type value
{
data_unset *} %type expression {
data_unset *} %type aelement {
data_unset *} %type condline {
data_config *} %type condlines {
data_config *} %type global {
data_config *} %type aelements {
array *} %type array {
array *} %type key {
buffer *} %type stringop {
buffer *} %type cond {
config_cond_t} %destructor value {
	$$->free($$);
}
		  ...
		  if (strncmp(A->ptr, "env.", sizeof("env.") - 1) == 0)
			{
				...
				ctx->ok = 0;
					 ...
					 array_get_element(ctx->current->value, B->key->ptr))
			{
	...
	if (strncmp(A->ptr, "env.", sizeof("env.") - 1) == 0)
	  {
		  fprintf(stderr,
				  ...
				  ctx->current->context_ndx, ctx->current->key->ptr, A->ptr);
		  ctx->ok = 0;
...
aelements(A):: = aelements(C)
COMMA aelement(B).
{
	if (buffer_is_empty(B->key) || NULL == array_get_element(C, B->key->ptr))
	  {
...
aelement(A):: = stringop(B)
ARRAY_ASSIGN expression(C).
{
...
eols:: = EOL.eols:: =.globalstart:: = GLOBAL.
{
	data_config *dc;
...
condlines(A):: = condlines(B)
eols ELSE condline(C).
{
...
context:: = DOLLAR SRVVARNAME(B)
LBRACKET stringop(C)
RBRACKET cond(E) expression(D).
{
	data_config *dc;
	buffer *b, *rvalue, *op;
	...
	if (NULL !=
		(dc = (data_config *) array_get_element(ctx->all_configs, b->ptr)))
	  {
		  configparser_push(ctx, dc, 0);
	} else
	  {
			...
			case CONFIG_COND_MATCH:
				{
					...
					if (NULL == (dc->regex =
								 pcre_compile(rvalue->ptr, 0, &errptr,
											  &erroff, NULL)))
					  {
