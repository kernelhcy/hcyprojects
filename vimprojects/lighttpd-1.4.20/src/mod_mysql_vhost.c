 ...
 */
static void *mod_mysql_vhost_connection_data(server * srv,
...
SERVER_FUNC(mod_mysql_vhost_set_defaults)
{
	...
	config_values_t cv[] = {
		...
		{"mysql-vhost.hostname", NULL, T_CONFIG_STRING,
	...
	for (i = 0; i < srv->config_context->used; i++)
	{
		...
		/*
		 * required: - username - database optional: - password, default:
		 * empty - socket, default: mysql default - hostname, if set overrides
		 * socket - port, default: 3306 
		 */

		/*
		 * all have to be set 
		 */
		if (!(buffer_is_empty(s->myuser) || buffer_is_empty(s->mydb)))
		{
			my_bool reconnect = 1;
			int fd;
...
#define FOO(x) (s->x->used ? s->x->ptr : NULL)
			...
			if (!mysql_real_connect
				(s->mysql, FOO(hostname), FOO(myuser), FOO(mypass),
...
#define PATCH(x) \
	p->conf.x = s->x;
static int
mod_mysql_vhost_patch_connection(server * srv, connection * con,
								 plugin_data * p)
{
	size_t i, j;
	...
	for (i = 1; i < srv->config_context->used; i++)
	{
		...
		for (j = 0; j < dc->value->used; j++)
		{
			...
			if (buffer_is_equal_string
				(du->key, CONST_STR_LEN("mysql-vhost.sql")))
			{
				PATCH(mysql_pre);
				PATCH(mysql_post);
			}
...
CONNECTION_FUNC(mod_mysql_vhost_handle_docroot)
{
	...
	if (c->server_name->used &&	/* con->uri.authority->used && */
		buffer_is_equal(c->server_name, con->uri.authority))
		goto GO_ON;
	...
	{
		log_error_write(srv, __FILE__, __LINE__, "sb", strerror(errno),
						p->tmp_buf);
		goto ERR500;
	}
	if (!S_ISDIR(sce->st.st_mode))
	{
		log_error_write(srv, __FILE__, __LINE__, "sb", "Not a directory",
						p->tmp_buf);
		goto ERR500;
	}
