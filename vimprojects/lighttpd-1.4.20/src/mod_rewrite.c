...
int
rewrite_rule_buffer_append(rewrite_rule_buffer * kvb, buffer * key,
						   buffer * value, int once)
{
#ifdef HAVE_PCRE_H
	size_t i;
	const char *errptr;
...
void rewrite_rule_buffer_free(rewrite_rule_buffer * kvb)
{
	...
	{
		if (kvb->ptr[i]->key)
			pcre_free(kvb->ptr[i]->key);
		if (kvb->ptr[i]->value)
			buffer_free(kvb->ptr[i]->value);
		free(kvb->ptr[i]);
	}
...
SETDEFAULTS_FUNC(mod_rewrite_set_defaults)
{
	...
	config_values_t cv[] = {
		{"url.rewrite-repeat", NULL, T_CONFIG_LOCAL,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 0 */
		{"url.rewrite-once", NULL, T_CONFIG_LOCAL,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 1 */

		/*
		 * old names, still supported url.rewrite remapped to url.rewrite-once
		 * url.rewrite-final is url.rewrite-once 
		 */
		{"url.rewrite", NULL, T_CONFIG_LOCAL, T_CONFIG_SCOPE_CONNECTION}
		,						/* 2 */
		{"url.rewrite-final", NULL, T_CONFIG_LOCAL,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 3 */
...
#ifdef HAVE_PCRE_H
static int
mod_rewrite_patch_connection(server * srv, connection * con, plugin_data * p)
{
	size_t i, j;
		...
		for (j = 0; j < dc->value->used; j++)
		{
			...
			if (buffer_is_equal_string(du->key, CONST_STR_LEN("url.rewrite")))
			{
				...
				p->conf.context = dc;
					   ...
					   (du->key, CONST_STR_LEN("url.rewrite-once")))
			{
				p->conf.rewrite = s->rewrite;
				p->conf.context = dc;
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("url.rewrite-repeat")))
			{
				p->conf.rewrite = s->rewrite;
				p->conf.context = dc;
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("url.rewrite-final")))
			{
				p->conf.rewrite = s->rewrite;
				p->conf.context = dc;
			}
...
# define N 10
		...
		if ((n =
			 pcre_exec(match, NULL, p->match_buf->ptr,
					   p->match_buf->used - 1, 0, 0, ovec, 3 * N)) < 0)
		{
		...
		} else
		{
			...
			for (k = 0; k < pattern_len; k++)
			{
				if (pattern[k] == '$' || pattern[k] == '%')
				{
					...
					buffer_append_string_len(con->request.uri,
					...
					{
						config_append_cond_match_buffer(con,
														p->conf.
														context,
														con->request.uri, num);
					}
			...
			buffer_append_string_len(con->request.uri, pattern + start,
									 pattern_len - start);
