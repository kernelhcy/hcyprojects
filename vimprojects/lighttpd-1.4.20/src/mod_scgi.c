...
typedef struct {
	...
	/*
	 * if socket is local we can start the scgi process ourself bin-path is
	...
	size_t max_id;				/* corresponds most of the time to num_procs.
								 ...
								 * the process itself to die and decrements its 
								 * afterwards */
} scgi_extension_host;
 ...
 */
typedef enum { FCGI_STATE_INIT, FCGI_STATE_CONNECT,
...
int scgi_proclist_sort_down(server * srv,
							scgi_extension_host * host, scgi_proc * proc);
...
int
scgi_extension_insert(scgi_exts * ext, buffer * key, scgi_extension_host * fh)
{
	scgi_extension *fe;
	size_t i;
			...
			for (j = 0; j < exts->used; j++)
			{
				...
				for (n = 0; n < ex->used; n++)
				{
					...
					for (proc = host->first; proc; proc = proc->next)
					{
						...
						if (proc->is_local && !buffer_is_empty(proc->socket))
						{
...
static int
scgi_spawn_connection(server * srv,
					  plugin_data * p,
					  scgi_extension_host * host, scgi_proc * proc)
{
	int scgi_fd;
	...
	} else
	{
		...
		} else
		{
			...
			memcpy(&(scgi_addr_in.sin_addr.s_addr), he->h_addr_list[0],
				   he->h_length);
	...
	if (-1 == connect(scgi_fd, scgi_addr, servlen))
	{
		...
		val = 1;
		if (setsockopt
			(scgi_fd, SOL_SOCKET, SO_REUSEADDR, &val, sizeof(val)) < 0)
		{
			log_error_write(srv, __FILE__, __LINE__, "ss",
		...
		if (-1 == bind(scgi_fd, scgi_addr, servlen))
		{
			log_error_write(srv, __FILE__, __LINE__, "sbds",
							"bind failed for:",
							proc->socket, proc->port, strerror(errno));
			return -1;
		}
		...
		case 0:
			{
				...
				log_error_write(srv, __FILE__, __LINE__, "sbs",
								"execl failed for:", host->bin_path,
								strerror(errno));
		...
		default:
			...
			default:
				...
				{
					log_error_write(srv, __FILE__, __LINE__, "sd",
									"child signaled:", WTERMSIG(status));
				} else
				{
					log_error_write(srv, __FILE__, __LINE__, "sd",
...
SETDEFAULTS_FUNC(mod_scgi_set_defaults)
{
	...
	config_values_t cv[] = {
		{"scgi.server", NULL, T_CONFIG_LOCAL, T_CONFIG_SCOPE_CONNECTION}
		,						/* 0 */
		{"scgi.debug", NULL, T_CONFIG_SHORT, T_CONFIG_SCOPE_CONNECTION}
		,						/* 1 */
			...
			for (j = 0; j < da->value->used; j++)
			{
				...
				for (n = 0; n < da_ext->value->used; n++)
				{
					...
					} else
					{
						...
						{
							buffer_copy_string_buffer(fp->socket,
													  df->unixsocket);
						}
...
static int
scgi_set_state(server * srv, handler_ctx * hctx, scgi_connection_state_t state)
{
	hctx->state = state;
		...
		if (hctx->got_proc)
		{
			...
			if (p->conf.debug)
			{
				log_error_write(srv, __FILE__, __LINE__, "sddb",
								"release proc:",
								hctx->fd, hctx->proc->pid, hctx->proc->socket);
			}
		}
...
static int scgi_reconnect(server * srv, handler_ctx * hctx)
{
	 ...
	 * setup a connection again if we have more then 5 reconnects for the same 
...
static handler_t
scgi_connection_reset(server * srv, connection * con, void *p_d)
{
	plugin_data *p = p_d;
...
static int
scgi_env_add(buffer * env, const char *key, size_t key_len,
	...
	if (-1 == connect(scgi_fd, scgi_addr, servlen))
	{
		...
		} else
		{
			log_error_write(srv, __FILE__, __LINE__, "sdsddb",
							...
							strerror(errno), errno, proc->port, proc->socket);
...
static int
scgi_env_add_request_headers(server * srv, connection * con, plugin_data * p)
{
	size_t i;
		...
		if (ds->value->used && ds->key->used)
		{
			...
			for (j = 0; j < ds->key->used - 1; j++)
			{
				srv->tmp_buf->ptr[srv->tmp_buf->used++] =
					light_isalpha(ds->key->ptr[j]) ? ds->key->
					ptr[j] & ~32 : '_';
			}
			srv->tmp_buf->ptr[srv->tmp_buf->used++] = '\0';

			scgi_env_add(p->scgi_env, CONST_BUF_LEN(srv->tmp_buf),
						 CONST_BUF_LEN(ds->value));
		}
	}
...
static int scgi_create_env(server * srv, handler_ctx * hctx)
{
	...
	scgi_env_add(p->scgi_env, CONST_STR_LEN("CONTENT_LENGTH"), buf,
				 strlen(buf));
	scgi_env_add(p->scgi_env, CONST_STR_LEN("SCGI"), CONST_STR_LEN("1"));


	scgi_env_add(p->scgi_env, CONST_STR_LEN("SERVER_SOFTWARE"),
	...
	scgi_env_add(p->scgi_env, CONST_STR_LEN("GATEWAY_INTERFACE"),
				 CONST_STR_LEN("CGI/1.1"));

	LI_ltostr(buf,
#ifdef HAVE_IPV6
			  ntohs(srv_sock->addr.plain.sa_family ? srv_sock->addr.ipv6.
	...
	LI_ltostr(buf,
#ifdef HAVE_IPV6
			  ntohs(con->dst_addr.plain.sa_family ? con->dst_addr.ipv6.
					sin6_port : con->dst_addr.ipv4.sin_port)
#else
	...
	scgi_env_add(p->scgi_env, CONST_STR_LEN("SCRIPT_NAME"),
				 CONST_BUF_LEN(con->uri.path));

	if (!buffer_is_empty(con->request.pathinfo))
	{
		scgi_env_add(p->scgi_env, CONST_STR_LEN("PATH_INFO"),
	...
	{
		scgi_env_add(p->scgi_env, CONST_STR_LEN("REDIRECT_URI"),
					 CONST_BUF_LEN(con->request.uri));
	}
	if (!buffer_is_empty(con->uri.query))
	{
		scgi_env_add(p->scgi_env, CONST_STR_LEN("QUERY_STRING"),
	...
	scgi_env_add(p->scgi_env, CONST_STR_LEN("REDIRECT_STATUS"), CONST_STR_LEN("200"));	/* if 
																						 * php 
																						 * is 
																						 * compiled 
																						 * with 
	...
	buffer_append_string_len(b, (const char *) p->scgi_env->ptr,
							 p->scgi_env->used);
	buffer_append_string_len(b, CONST_STR_LEN(","));
	...
	if (con->request.content_length)
	{
			 ...
			 req_c = req_c->next)
		{
			...
			/*
			 * we announce toWrite octects now take all the request_content
			 * chunk that we need to fill this request 
			 */
			...
			case FILE_CHUNK:
				...
				chunkqueue_append_file(hctx->wb, req_c->file.name,
									   req_c->offset, weHave);
			...
			case MEM_CHUNK:
				...
				buffer_append_memory(b,
									 req_c->mem->ptr + req_c->offset, weHave);
...
static int
scgi_response_parse(server * srv, connection * con, plugin_data * p,
					buffer * in, int eol)
{
	char *ns;
	const char *s;
		 ...
		 s = ns + (eol == EOL_RN ? 2 : 1), line++)
	{
		...
		if (line == 0 && 0 == strncmp(s, "HTTP/1.", 7))
		{
			/*
		...
		} else
		{
			...
			if (NULL ==
				(ds =
				 (data_string *) array_get_unused_element(con->response.
														  headers,
														  TYPE_STRING)))
			{
...
static int scgi_demux_response(server * srv, handler_ctx * hctx)
{
	...
	while (1)
	{
		...
		if (con->file_started == 0)
		{
			 ...
			 */
			if (0 == strncmp(hctx->response_header->ptr, "HTTP/1.", 7))
				in_header = 1;
			...
			for (c = hctx->response_header->ptr, cp = 0, used =
				 hctx->response_header->used - 1; used; c++, cp++, used--)
			{
				if (*c == ':')
					in_header = 1;
				else if (*c == '\n')
			...
			if (header_end)
			{
				if (c == NULL)
				{
					...
					http_chunk_append_mem(srv, con,
										  hctx->response_header->ptr,
										  hctx->response_header->used);
					joblist_append(srv, con);
				} else
				{
					size_t hlen =
						c - hctx->response_header->ptr + (eol ==
														  EOL_RN ? 4 : 2);
					 ...
					 */
					hctx->response_header->used =
						hlen + 1 - (eol == EOL_RN ? 2 : 1);
					hctx->response_header->ptr[hlen -
											   (eol == EOL_RN ? 2 : 1)] = '\0';

					/*
					 * parse the response header 
					 */
					scgi_response_parse(srv, con, p,
										hctx->response_header, eol);
...
#if 0
		log_error_write(srv, __FILE__, __LINE__, "ddss", con->fd, hctx->fd,
...
int
scgi_proclist_sort_up(server * srv, scgi_extension_host * host,
					  scgi_proc * proc)
{
	scgi_proc *p;
	...
	/*
	 * we have been the smallest of the current list and we want to insert the
	 * node sorted as soon possible 1 0 0 0 1 1 1 | ^ | | +------+ 
	 */

	/*
	 * nothing to sort, only one element 
	 */
	if (host->first == proc && proc->next == NULL)
		return 0;

	for (p = proc; p->next && p->next->load < proc->load; p = p->next);

	/*
	 * no need to move something 1 2 2 2 3 3 3 ^ | + 
	 */
	if (p == proc)
		return 0;
	...
	{
		log_error_write(srv, __FILE__, __LINE__, "dd", p->pid, p->load);
	}
#else
	UNUSED(srv);
#endif
...
int
scgi_proclist_sort_down(server * srv, scgi_extension_host * host,
						scgi_proc * proc)
{
	scgi_proc *p;
	...
	/*
	 * we have been the smallest of the current list and we want to insert the
	 * node sorted as soon possible 0 0 0 0 1 0 1 ^ | | | +----------+ the
	 * basic is idea is: - the last active scgi process should be still in ram
	 * and is not swapped out yet - processes that are not reused will be
	 * killed after some time by the trigger-handler - remember it as:
	 * everything > 0 is hot all unused procs are colder the more right they
	 * are ice-cold processes are propably unused since more than
	 * 'unused-timeout', are swaped out and won't be reused in the next seconds 
	 * anyway. 
	 */
	...
	/*
	 * we have to move left. If we are already the first element we are done 
	 */
	if (host->first == proc)
		return 0;
...
static int
scgi_restart_dead_procs(server * srv, plugin_data * p,
						scgi_extension_host * host)
{
	scgi_proc *proc;
	...
	for (proc = host->first; proc; proc = proc->next)
	{
		...
		} else
		{
			 ...
			 *
			 */
			if (proc->state == PROC_STATE_DIED && proc->load == 0)
			{
				/*
				 * restart the child 
				 */
			...
			if (errno == ENOTCONN)
			{
				...
				if (hctx->wb->bytes_out == 0 && hctx->reconnects < 5)
				{
					usleep(10000);	/* take away the load of the webserver to
									 * let the php a chance to restart */
...
SUBREQUEST_FUNC(mod_scgi_handle_subrequest)
{
	...
	case HANDLER_ERROR:
		...
		if (proc && 0 == proc->is_local && proc->state != PROC_STATE_DISABLED)
		{
			/*
			...
			log_error_write(srv, __FILE__, __LINE__, "sbdb",
							"fcgi-server disabled:", host->host,
		...
		if (hctx->state == FCGI_STATE_INIT || hctx->state == FCGI_STATE_CONNECT)
		{
			/*
			 * connect() or getsockopt() failed, restart the request-handling 
			 */
			if (proc && proc->is_local)
			{
				...
				{
					log_error_write(srv, __FILE__, __LINE__, "sbdb",
									"connect() to scgi failed, restarting the request-handling:",
				 ...
				 *
				 */
				if (proc->state == PROC_STATE_RUNNING && hctx->pid == proc->pid)
				{
			...
			/*
			 * mis-using HANDLER_WAIT_FOR_FD to break out of the loop and hope
			 * that the childs will be restarted 
			 */
			return HANDLER_WAIT_FOR_FD;
		} else
		{
...
static handler_t scgi_handle_fdevent(void *s, void *ctx, int revents)
{
		...
		case -1:
			...
			if (con->file_started == 0)
			{
				...
				log_error_write(srv, __FILE__, __LINE__, "sosdsd",
								"response not sent, request sent:",
								hctx->wb->bytes_out, "connection-fd:",
								con->fd, "fcgi-fd:", hctx->fd);
	...
	if (revents & FDEVENT_OUT)
	{
		...
		{
			/*
			 * we are allowed to send something out 1. in a unfinished
			 * connect() call 2. in a unfinished write() call (long POST
			 * request) 
			 */
	...
	if (revents & FDEVENT_HUP)
	{
		if (hctx->state == FCGI_STATE_CONNECT)
		{
			 ...
			 * we might get a EINPROGRESS in the first call and a FDEVENT_HUP
...
#define PATCH(x) \
	p->conf.x = s->x;
static int
scgi_patch_connection(server * srv, connection * con, plugin_data * p)
{
	size_t i, j;
		...
		for (j = 0; j < dc->value->used; j++)
		{
			...
			if (buffer_is_equal_string(du->key, CONST_STR_LEN("scgi.server")))
			{
				PATCH(exts);
					   ...
					   (du->key, CONST_STR_LEN("scgi.debug")))
			{
				PATCH(debug);
			}
		}
...
static handler_t
scgi_check_extension(server * srv, connection * con, void *p_d,
		...
		if (*(extension->key->ptr) == '/')
		{
			if (strncmp(fn->ptr, extension->key->ptr, ct_len) == 0)
				break;
				   ...
				   strncmp(fn->ptr + s_len - ct_len, extension->key->ptr,
						   ct_len))
		{
			/*
 ...
 */
static handler_t
scgi_check_extension_1(server * srv, connection * con, void *p_d)
{
 ...
 */
static handler_t
scgi_check_extension_2(server * srv, connection * con, void *p_d)
{
...
static handler_t
scgi_connection_close_callback(server * srv, connection * con, void *p_d)
{
	plugin_data *p = p_d;
...
TRIGGER_FUNC(mod_scgi_handle_trigger)
{
	 ...
	 * we wait for the TCP timeout which is on Linux 180 seconds 
	 */
		...
		for (j = 0; j < exts->used; j++)
		{
			...
			for (n = 0; n < ex->used; n++)
			{
					...
					(sum_load / host->num_procs) > host->max_load_per_proc)
				{
					...
					for (fp = host->unused_procs; fp && fp->pid != 0;
						 fp = fp->next);

					if (fp)
					{
						if (fp == host->unused_procs)
							host->unused_procs = fp->next;
					...
					} else
					{
						...
						buffer_append_string_len(fp->socket,
												 CONST_STR_LEN("-"));
				...
				for (proc = host->first; proc; proc = proc->next)
				{
					...
					{
						/*
						 * a proc is idling for a long time now, terminated it 
						 */
