...
SETDEFAULTS_FUNC(mod_extforward_set_defaults)
{
	...
	config_values_t cv[] = {
		{"extforward.forwarder", NULL, T_CONFIG_ARRAY,
...
#define PATCH(x) \
	p->conf.x = s->x;
static int
mod_extforward_patch_connection(server * srv, connection * con, plugin_data * p)
{
	size_t i, j;
	...
	for (i = 1; i < srv->config_context->used; i++)
	{
		...
		for (j = 0; j < dc->value->used; j++)
		{
			...
			if (buffer_is_equal_string
				(du->key, CONST_STR_LEN("extforward.forwarder")))
			{
				PATCH(forwarder);
			}
		}
	...
	if (pbuffer->used > 0)
	{
		...
		for (base = pbuffer->ptr, curr = pbuffer->ptr; *curr; curr++)
		{
			...
			} else
			{
				...
				{
					/*
					 * found leading char of an IP address, move base pointer
					 * and change state 
					 */
					base = curr;
					in_str = 1;
...
{
	data_string *allds =
		(data_string *) array_get_element(p->conf.forwarder, "all");
	...
	return (data_string *) array_get_element(p->conf.forwarder,
											 ipstr) ? IP_TRUSTED : IP_UNTRUSTED;
}
...
#define AI_NUMERICSERV 0
	...
	if (result != 0)
	{
		...
		fprintf(stderr,
				"Problem in resolving hostname %s: succeeded, but no information returned\n",
				host);
...
URIHANDLER_FUNC(mod_extforward_uri_handler)
{
	...
	dst_addr_str = inet_ntop(con->dst_addr.plain.sa_family,
							 ...
							 (struct sockaddr *) &(con->dst_addr.ipv6.
												   sin6_addr) : (struct
																 sockaddr *)
							 &(con->dst_addr.ipv4.sin_addr), b2,
							 (sizeof b2) - 1);
#else
	...
	if (real_remote_addr != NULL)
	{							/* parsed */
		sock_addr sock;
		struct addrinfo *addrs_left;
		server_socket *srv_sock = con->srv_socket;
		data_string *forwarded_proto =
			(data_string *) array_get_element(con->request.headers,
											  "X-Forwarded-Proto");
			 ...
			 addrs_left = addrs_left->ai_next)
		{
			...
			{
				sock.ipv4.sin_addr =
					((struct sockaddr_in *) addrs_left->ai_addr)->sin_addr;
				break;
			} else if (sock.plain.sa_family == AF_INET6)
			{
				sock.ipv6.sin6_addr =
					((struct sockaddr_in6 *) addrs_left->ai_addr)->sin6_addr;
				break;
			}
		}
#else
		sock.ipv4.sin_addr.s_addr = inet_addr(real_remote_addr);
		sock.plain.sa_family =
			(sock.ipv4.sin_addr.s_addr == 0xFFFFFFFF) ? AF_UNSPEC : AF_INET;
#endif
		if (sock.plain.sa_family != AF_UNSPEC)
		{
			/*
			 * we found the remote address, modify current connection and save
			 * the old address 
			 */
			 ...
			 */
			con->plugin_ctx[p->id] =
				handler_ctx_init(con->dst_addr, con->dst_addr_buf);
			/*
			...
			{
				log_error_write(srv, __FILE__, __LINE__, "ss",
								"patching con->dst_addr_buf for the accesslog:",
								real_remote_addr);
			}
			/*
			 * Now, clean the conf_cond cache, because we may have changed the
			 * results of tests 
			 */
...
CONNECTION_FUNC(mod_extforward_restore)
{
	...
	/*
	 * Now, clean the conf_cond cache, because we may have changed the results
	 * of tests 
	 */
	clean_cond_cache(srv, con);
