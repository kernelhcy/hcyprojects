...
int ssi_env_add(array * env, const char *key, const char *val)
{
	...
	if (NULL ==
		(ds = (data_string *) array_get_unused_element(env, TYPE_STRING)))
	{
		ds = data_string_init();
	}
...
static int
ssi_env_add_request_headers(server * srv, connection * con, plugin_data * p)
{
	size_t i;
...
#define CONST_STRING(x) \
	...
	ssi_env_add(p->ssi_cgi_env, CONST_STRING("SERVER_SOFTWARE"),
	...
	ssi_env_add(p->ssi_cgi_env, CONST_STRING("SCRIPT_NAME"),
				con->uri.path->ptr);
	...
	{
		ssi_env_add(p->ssi_cgi_env, CONST_STRING("PATH_INFO"),
					con->request.pathinfo->ptr);
	}

	ssi_env_add(p->ssi_cgi_env, CONST_STRING("SCRIPT_FILENAME"),
				con->physical.path->ptr);
	ssi_env_add(p->ssi_cgi_env, CONST_STRING("DOCUMENT_ROOT"),
				con->physical.doc_root->ptr);

	ssi_env_add(p->ssi_cgi_env, CONST_STRING("REQUEST_URI"),
				con->request.uri->ptr);
	ssi_env_add(p->ssi_cgi_env, CONST_STRING("QUERY_STRING"),
				con->uri.query->used ? con->uri.query->ptr : "");
	ssi_env_add(p->ssi_cgi_env, CONST_STRING("REQUEST_METHOD"),
				get_http_method_name(con->request.http_method));
	ssi_env_add(p->ssi_cgi_env, CONST_STRING("REDIRECT_STATUS"), "200");
	ssi_env_add(p->ssi_cgi_env, CONST_STRING("SERVER_PROTOCOL"),
...
static int
process_ssi_stmt(server * srv, connection * con, plugin_data * p,
				 const char **l, size_t n)
{
	...
	case SSI_ECHO:
		{
			...
			struct {
				const char *var;
				enum { SSI_ECHO_UNSET, SSI_ECHO_DATE_GMT,
					SSI_ECHO_DATE_LOCAL,
			...
			struct {
				const char *var;
				enum { SSI_ENC_UNSET, SSI_ENC_URL, SSI_ENC_NONE,
					SSI_ENC_ENTITY
				} type;
			} encvars[] =
			{
			...
			for (i = 2; i < n; i += 2)
			{
				...
				} else
				{
					log_error_write(srv, __FILE__, __LINE__, "sss",
									"ssi: unknow attribute for ", l[1], l[i]);
				}
			}
			...
			case SSI_ECHO_LAST_MODIFIED:
				{
					...
					b = chunkqueue_get_append_buffer(con->write_queue);
					if (0 ==
						strftime(buf, sizeof(buf), p->timefmt->ptr,
								 localtime(&t)))
					{
			...
			case SSI_ECHO_DATE_GMT:
				{
					...
					b = chunkqueue_get_append_buffer(con->write_queue);
					if (0 ==
						strftime(buf, sizeof(buf), p->timefmt->ptr, gmtime(&t)))
					{
						buffer_copy_string_len(b, CONST_STR_LEN("(none)"));
					} else
					{
			...
			default:
				{
					...
					if (NULL !=
						(ds =
						 (data_string *) array_get_element(p->
														   ssi_cgi_env,
														   var_val)))
					{
	...
	case SSI_FSIZE:
		{
			...
			{
				log_error_write(srv, __FILE__, __LINE__, "sss",
								"ssi: ", l[1], "file or virtual are missing");
				break;
			}
			...
			if (file_path && virt_path)
			{
				log_error_write(srv, __FILE__, __LINE__, "sss",
								"ssi: ",
								l[1],
								"only one of file and virtual is allowed here");
				break;
			}
			...
			if (file_path)
			{
				...
				{
					buffer_copy_string_len(p->stat_fn,
										   con->physical.path->ptr,
										   sl - con->physical.path->ptr + 1);
				}
	...
	case SSI_SET:
		{
			...
			if (key && val)
			{
				...
				if (NULL ==
					(ds =
					 (data_string *) array_get_unused_element(p->
															  ssi_vars,
															  TYPE_STRING)))
				{
					ds = data_string_init();
	...
	case SSI_PRINTENV:
		...
		{
			data_string *ds =
				(data_string *) p->ssi_vars->data[p->ssi_vars->sorted[i]];
	...
	case SSI_EXEC:
		{
			...
			/*
			 * create a return pipe and send output to the html-page as exec
			 * is assumed evil it is implemented synchronously 
			 */
			...
			default:
				{
					...
					do
					{
						...
						} else if (WIFEXITED(status))
						{
							int toread;
							/*
							 * read everything from client and paste it into
							 * the output 
							 */
							was_interrupted = 0;
					...
					while (was_interrupted > 0 && was_interrupted < 4);	/* if
																		 * waitpid() 
																		 * gets 
																		 * interrupted, 
																		 * retry, 
	...
	case SSI_ELIF:
		{
			...
			if (p->if_level == p->if_is_false_level)
			{
				if ((p->if_is_false) && (p->if_is_false_endif == 0))
				{
					switch (ssi_eval_expr(srv, con, p, expr))
					{
					case -1:
...
static int
mod_ssi_handle_request(server * srv, connection * con, plugin_data * p)
{
	stream s;
#ifdef  HAVE_PCRE_H
	int i, n;
...
#define N 10
	...
	array_reset(p->ssi_cgi_env);
	buffer_copy_string_len(p->timefmt,
	...
	for (i = 0;
		 (n =
		  pcre_exec(p->ssi_regex, NULL, s.start, s.size, i, 0, ovec,
					N * 3)) > 0; i = ovec[1])
	{
		const char **l;
		/*
		...
		if (!p->if_is_false)
			chunkqueue_append_file(con->write_queue, con->physical.path, i,
								   ovec[0] - i);
...
#define PATCH(x) \
	p->conf.x = s->x;
static int
mod_ssi_patch_connection(server * srv, connection * con, plugin_data * p)
{
	size_t i, j;
...
URIHANDLER_FUNC(mod_ssi_physical_path)
{
	...
	for (k = 0; k < p->conf.ssi_extension->used; k++)
	{
		...
		if (buffer_is_equal_right_len
			(con->physical.path, ds->value, ds->value->used - 1))
		{
			/*
			 * handle ssi-request 
			 */
