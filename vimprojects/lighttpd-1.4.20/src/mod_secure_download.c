...
#define PATCH(x) \
	p->conf.x = s->x;
static int
mod_secdownload_patch_connection(server * srv, connection * con,
								 plugin_data * p)
{
	size_t i, j;
	...
	for (i = 1; i < srv->config_context->used; i++)
	{
		...
		for (j = 0; j < dc->value->used; j++)
		{
			...
			if (buffer_is_equal_string
				(du->key, CONST_STR_LEN("secdownload.secret")))
			{
				PATCH(secret);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("secdownload.document-root")))
			{
				PATCH(doc_root);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("secdownload.uri-prefix")))
			{
				PATCH(uri_prefix);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN("secdownload.timeout")))
			{
				PATCH(timeout);
			}
		}
...
URIHANDLER_FUNC(mod_secdownload_uri_handler)
{
	...
	if (buffer_is_empty(p->conf.uri_prefix))
		return HANDLER_GO_ON;
	...
	if (0 !=
		strncmp(con->uri.path->ptr, p->conf.uri_prefix->ptr,
				p->conf.uri_prefix->used - 1))
		return HANDLER_GO_ON;
	...
	/*
	 * checking MD5 <secret><rel-path><timestamp-hex> 
	 */
