...
static inline HANDLE dup_handle(HANDLE src, BOOL inherit, BOOL closeorig)
{
	...
	if (!DuplicateHandle
		(self, src, self, &copy, 0, inherit,
		 DUPLICATE_SAME_ACCESS | (closeorig ? DUPLICATE_CLOSE_SOURCE : 0)))
		return NULL;
	return copy;
}
...
int proc_open(proc_handler_t * proc, const char *command)
{
	...
	{
		fprintf(stderr, "One of %s,%%SystemRoot,%%windir is required",
				SHELLENV);
		return -1;
	}
 ...
 */
int
proc_open_buffer(proc_handler_t * proc, const char *command, buffer * in,
				 buffer * out, buffer * err)
{
