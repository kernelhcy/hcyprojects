...
void excludes_buffer_free(excludes_buffer * exb)
{
	...
	{
		if (exb->ptr[i]->regex)
			pcre_free(exb->ptr[i]->regex);
		if (exb->ptr[i]->string)
			buffer_free(exb->ptr[i]->string);
		free(exb->ptr[i]);
	}
...
static int
parse_config_entry(server * srv, plugin_config * s, array * ca,
				   const char *option)
{
	data_unset *du;
...
SETDEFAULTS_FUNC(mod_dirlisting_set_defaults)
{
	...
	config_values_t cv[] = {
		{CONFIG_EXCLUDE, NULL, T_CONFIG_LOCAL, T_CONFIG_SCOPE_CONNECTION}
		,						/* 0 */
		{CONFIG_ACTIVATE, NULL, T_CONFIG_BOOLEAN, T_CONFIG_SCOPE_CONNECTION}
		,						/* 1 */
		{CONFIG_HIDE_DOTFILES, NULL, T_CONFIG_BOOLEAN,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 2 */
		{CONFIG_EXTERNAL_CSS, NULL, T_CONFIG_STRING,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 3 */
		{CONFIG_ENCODING, NULL, T_CONFIG_STRING, T_CONFIG_SCOPE_CONNECTION}
		,						/* 4 */
		{CONFIG_SHOW_README, NULL, T_CONFIG_BOOLEAN,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 5 */
		{CONFIG_HIDE_README_FILE, NULL, T_CONFIG_BOOLEAN,
		 T_CONFIG_SCOPE_CONNECTION}
		,						/* 6 */
...
#define PATCH(x) \
	p->conf.x = s->x;
static int
mod_dirlisting_patch_connection(server * srv, connection * con, plugin_data * p)
{
	size_t i, j;
	...
	for (i = 1; i < srv->config_context->used; i++)
	{
		...
		for (j = 0; j < dc->value->used; j++)
		{
			...
			if (buffer_is_equal_string
				(du->key, CONST_STR_LEN(CONFIG_ACTIVATE))
				|| buffer_is_equal_string(du->key,
										  CONST_STR_LEN(CONFIG_DIR_LISTING)))
			{
				PATCH(dir_listing);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN(CONFIG_HIDE_DOTFILES)))
			{
				PATCH(hide_dot_files);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN(CONFIG_EXTERNAL_CSS)))
			{
				PATCH(external_css);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN(CONFIG_ENCODING)))
			{
				PATCH(encoding);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN(CONFIG_SHOW_README)))
			{
				PATCH(show_readme);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN(CONFIG_HIDE_README_FILE)))
			{
				PATCH(hide_readme_file);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN(CONFIG_SHOW_HEADER)))
			{
				PATCH(show_header);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN(CONFIG_HIDE_HEADER_FILE)))
			{
				PATCH(hide_header_file);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN(CONFIG_SET_FOOTER)))
			{
				PATCH(set_footer);
			} else if (buffer_is_equal_string
					   (du->key, CONST_STR_LEN(CONFIG_EXCLUDE)))
			{
				PATCH(excludes);
			}
		}
...
/*
 * buffer must be able to hold "999.9K" conversion is simple but not perfect 
 */
...
static void
http_list_directory_header(server * srv, connection * con, plugin_data * p,
						   buffer * out)
{
	UNUSED(srv);

	buffer_append_string_len(out,
							 CONST_STR_LEN
							 ("<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.1//EN\" \"http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd\">\n"
							  "<html xmlns=\"http://www.w3.org/1999/xhtml\" xml:lang=\"en\">\n"
							  "<head>\n" "<title>Index of "));
	buffer_append_string_encoded(out, CONST_BUF_LEN(con->uri.path),
	...
	} else
	{
		buffer_append_string_len(out,
								 ...
								 ("<style type=\"text/css\">\n"
								  ...
								  "div.list {" " background-color: white;"
								  " border-top: 1px solid #646464;"
								  " border-bottom: 1px solid #646464;"
								  " padding-top: 10px;"
								  " padding-bottom: 14px;" "}\n"
								  "div.foot {" " font: 90% monospace;"
	...
	if (p->conf.show_header)
	{
		...
		if (-1 != stream_open(&s, p->tmp_buf))
		{
			...
			buffer_append_string_encoded(out, s.start, s.size,
										 ENCODING_MINIMAL_XML);
...
static void
http_list_directory_footer(server * srv, connection * con, plugin_data * p,
						   buffer * out)
{
	UNUSED(srv);
...
static int
http_list_directory(server * srv, connection * con, plugin_data * p,
					buffer * dir)
{
	DIR *dp;
	buffer *out;
	...
	while ((dent = readdir(dp)) != NULL)
	{
		...
		/*
		 * compare d_name against excludes array elements, skipping any that
		 * match. 
		 */
#ifdef HAVE_PCRE_H
...
#define N 10
		...
		/*
		 * NOTE: the manual says, d_name is never more than NAME_MAX so this
		 * should actually not be a buffer-overflow-risk 
		 */
		if (i > (size_t) name_max)
			continue;
		...
		if (list->used == list->size)
		{
			...
			list->ent =
				(dirls_entry_t **) realloc(list->ent,
	...
	if (dirs.used)
		http_dirls_sort(dirs.ent, dirs.used);

	if (files.used)
		http_dirls_sort(files.ent, files.used);

	out = chunkqueue_get_append_buffer(con->write_queue);
	buffer_copy_string_len(out,
	...
	for (i = 0; i < dirs.used; i++)
	{
		...
		buffer_append_string_encoded(out, DIRLIST_ENT_NAME(tmp),
									 tmp->namelen, ENCODING_REL_URI_PART);
		buffer_append_string_len(out, CONST_STR_LEN("/\">"));
		buffer_append_string_encoded(out, DIRLIST_ENT_NAME(tmp),
									 tmp->namelen, ENCODING_MINIMAL_XML);
		buffer_append_string_len(out,
	...
	for (i = 0; i < files.used; i++)
	{
		...
		if (content_type == NULL)
		{
			...
			for (k = 0; k < con->conf.mimetypes->used; k++)
			{
				...
				if (0 ==
					strncasecmp(DIRLIST_ENT_NAME(tmp) + tmp->namelen -
								ct_len, ds->key->ptr, ct_len))
				{
...
URIHANDLER_FUNC(mod_dirlisting_subrequest)
{
	...
	if (con->conf.log_request_handling)
	{
		...
		log_error_write(srv, __FILE__, __LINE__, "sb", "URI          :",
						con->uri.path);
	}
