...
SETDEFAULTS_FUNC(mod_flv_streaming_set_defaults)
{
	...
	config_values_t cv[] = {
		{"flv-streaming.extensions", NULL, T_CONFIG_ARRAY,
...
#define PATCH(x) \
	p->conf.x = s->x;
static int
mod_flv_streaming_patch_connection(server * srv, connection * con,
								   plugin_data * p)
{
	size_t i, j;
	...
	for (i = 1; i < srv->config_context->used; i++)
	{
		...
		for (j = 0; j < dc->value->used; j++)
		{
			...
			if (buffer_is_equal_string
				(du->key, CONST_STR_LEN("flv-streaming.extensions")))
			{
				PATCH(extensions);
			}
		}
		...
		case '\0':				/* fin symbol */
			if (!is_key)
			{
				...
				if (NULL == (ds = (data_string *)
							 array_get_unused_element(get_params, TYPE_STRING)))
				{
					ds = data_string_init();
				}
...
URIHANDLER_FUNC(mod_flv_streaming_path_handler)
{
	...
	for (k = 0; k < p->conf.extensions->used; k++)
	{
		...
		if (0 ==
			strncmp(con->physical.path->ptr + s_len - ct_len,
					ds->value->ptr, ct_len))
		{
			...
			if (NULL ==
				(get_param =
				 (data_string *) array_get_element(p->get_params, "start")))
			{
				return HANDLER_GO_ON;
			}
			...
			http_chunk_append_file(srv, con, con->physical.path, start,
								   sce->st.st_size - start);

			response_header_overwrite(srv, con,
									  CONST_STR_LEN("Content-Type"),
									  CONST_STR_LEN("video/x-flv"));
