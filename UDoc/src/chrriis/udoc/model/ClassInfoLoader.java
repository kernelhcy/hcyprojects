...
//    String docPath = className.replace('.', '/') + ".xml";
...
//    }
    try {
      ...
      // Prototype should be first, so no real impact on performace by running the loop twice.
  ...
  protected static FieldInfo loadFieldInfo(ClassInfo classInfo, String prototype, ClassProcessor classProcessor) {
    ...
    while(prototype.startsWith("@") && !prototype.startsWith("@interface ")) {
      ...
      for(int i=0; i<prototype.length(); i++) {
        char c = prototype.charAt(i);
        switch(c) {
          case '(': count++; sb.append(c); hasParameters = true; break;
          case ')': count--; sb.append(c); break;
          case ' ':
            if(count == 0) {
              prototype = prototype.substring(i + 1);
...
//      parametersParameterInfo = new FieldInfo[parameters.length];
