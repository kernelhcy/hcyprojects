...
public class ClassComponent extends JPanel {
  ...
  public ClassComponent(ClassPane classPane, ClassInfo classInfo) {
    ...
    showSuperTypesLabel.addMouseListener(new MouseAdapter() {
      public void mousePressed(MouseEvent e) {
        if(e.getButton() != MouseEvent.BUTTON1 || ClassComponent.this.classPane.isLocked() && !ClassComponent.this.classInfo.isLoaded()) {
          return;
        }
    ...
    MouseInputAdapter resizingMouseEvent = new MouseInputAdapter() {
      ...
      public void mousePressed(MouseEvent e) {
        ...
        location = SwingUtilities.convertPoint(resizingPanel, p, ClassComponent.this.classPane);
      }
  ...
  protected void setSuperTypesIndication(boolean areSuperTypesVisible) {
    ...
    if(!isClassLoaded || classInfo.getSuperTypes().length != 0) {
      showSuperTypesLabel.setIcon(IconManager.getIcon(areSuperTypesVisible? "super_types_on.gif": "super_types_off.gif"));
    }
  }
  ...
  protected void setSubTypesIndication(boolean areSubTypesVisible) {
    ...
    showSubTypesLabel.setIcon(IconManager.getIcon(areSubTypesVisible? "sub_types_on.gif": "sub_types_off.gif"));
  }
  ...
  protected void setCompositionsIndication(boolean areCompositionsVisible) {
    ...
    showCompositionsLabel.setIcon(IconManager.getIcon(areCompositionsVisible? "compositions_on.gif":  "compositions_off.gif"));
  }
  ...
  protected void setAssociationsIndication(boolean areAssociationsVisible) {
    ...
    showAssociationsLabel.setIcon(IconManager.getIcon(areAssociationsVisible? "associations_on.gif": "associations_off.gif"));
  }
  ...
  public void setExpanded(boolean isExpanded) {
    ...
    setSize(expandedPreferredSize != null && isExpanded? expandedPreferredSize: getPreferredSize());
    revalidate();
    repaint();
  }
  ...
  class MouseHandler extends MouseInputAdapter {
    ...
    public void mouseDragged(MouseEvent e) {
      ...
      classPane.moveSelectedClassComponent(ClassComponent.this, e.getX() - referenceLocation.x, e.getY() - referenceLocation.y);
    }
    public void mousePressed(MouseEvent e) {
      if(e.getButton() != MouseEvent.BUTTON1) {
        return;
...
//      System.err.println("2");
      if(!isForced && (e.getModifiers() & (MouseEvent.BUTTON1_MASK | MouseEvent.BUTTON2_MASK | MouseEvent.BUTTON3_MASK)) != 0) {
        return;
      }
          ...
          if(component instanceof ClassLink) {
            ...
            if(classInfo.isLoaded()) {
              ...
              if(prototype != null) {
                ...
                classPane.showTooltip(tooltip, classLink, (classLink.getWidth() - tooltip.getPreferredSize().width) / 2, classLink.getHeight() + 20);
              }
            }
            ...
            if(prototype != null) {
              ...
              classPane.showTooltip(tooltip, methodLink, (methodLink.getWidth() - tooltip.getPreferredSize().width) / 2, methodLink.getHeight() + 20);
            }
            ...
            if(prototype != null) {
              ...
              classPane.showTooltip(tooltip, fieldLink, (fieldLink.getWidth() - tooltip.getPreferredSize().width) / 2, fieldLink.getHeight() + 20);
            }
          }
        }
      }
  ...
  class InnerPanel extends JPanel {
    ...
    public InnerPanel(ClassInfo classInfo) {
      ...
      expandLabel = new JLabel(IconManager.getIcon("expand.gif")) {
        public boolean isVisible() {
          if(ClassComponent.this.classPane.isLocked() && !InnerPanel.this.classInfo.isLoaded()) {
            return false;
          }
      ...
      expandLabel.addMouseListener(new MouseAdapter() {
        public void mousePressed(MouseEvent e) {
          if(e.getButton() != MouseEvent.BUTTON1 || ClassComponent.this.classPane.isLocked() && !InnerPanel.this.classInfo.isLoaded()) {
            return;
          }
      ...
      titleLabel = new JLabel(IconManager.getIcon(Modifiers.NOT_LOADED | classInfo.getModifiers()));
    ...
    public void setExpanded(boolean isExpanded) {
      if(!hasClassContent() && (isClassLoaded || (classInfo.getModifiers() & Modifiers.LOADING_FAILED) != 0)) {
        isExpanded = false;
      }
      expandLabel.setIcon(isExpanded? IconManager.getIcon("collapse.gif"): IconManager.getIcon("expand.gif"));
    ...
    public void updateMembersPanel() {
      ...
      MethodInfo[] constructors = filterMethodInfos(classInfo.getConstructors(), methodVisibility);
      MethodInfo[] methods = filterMethodInfos(classInfo.getMethods(), methodVisibility);
      MethodInfo[] annotationMembers = filterMethodInfos(classInfo.getAnnotationMembers(), methodVisibility);
      ...
      for(int i=0; i<enums.length; i++) {
        membersPanel.add(new FieldDeclarationComponent(ClassComponent.this, classInfo, enums[i]));
        hasAdded = true;
      }
      if(fields.length > 0) {
        if(hasAdded) {
        ...
        for(int i=0; i<fields.length; i++) {
          membersPanel.add(new FieldDeclarationComponent(ClassComponent.this, classInfo, fields[i]));
          hasAdded = true;
        }
      }
      if(constructors.length > 0) {
        ...
        for(int i=0; i<constructors.length; i++) {
          membersPanel.add(new MethodDeclarationComponent(ClassComponent.this, classInfo, constructors[i]));
          hasAdded = true;
        }
      }
      if(methods.length > 0) {
        ...
        for(int i=0; i<methods.length; i++) {
          membersPanel.add(new MethodDeclarationComponent(ClassComponent.this, classInfo, methods[i]));
          hasAdded = true;
        }
      }
      if(annotationMembers.length > 0) {
        ...
        for(int i=0; i<annotationMembers.length; i++) {
          membersPanel.add(new MethodDeclarationComponent(ClassComponent.this, classInfo, annotationMembers[i]));
          hasAdded = true;
        }
      }
    ...
    public void setClassLoading() {
      titleLabel.setIcon(IconManager.getIcon(Modifiers.LOADING | classInfo.getModifiers() & ~Modifiers.LOADING_FAILED));
    }

  }

  protected boolean hasClassContent() {
    return classInfo.getEnums().length != 0 || classInfo.getAnnotationMembers().length != 0 || classInfo.getFields().length != 0 || classInfo.getConstructors().length != 0 || classInfo.getMethods().length != 0;
  }
  ...
  public Point getRelationPoint(Point p2) {
    ...
    pBounds = SwingUtilities.convertRectangle(packageComponent.getParent(), pBounds, getParent());
    pBounds.x--;
    pBounds.y--;
    pBounds.width++;
  ...
  protected Point getIntersection(Point p1, Point p2, Point p3, Point p4) {
    ...
    float ua = uan / uad;
    Point p = new Point(Math.round(p1.x + ua * (p2.x - p1.x)), Math.round(p1.y + ua * (p2.y - p1.y)));
    if(p.x < p1.x && p.x < p2.x || p.x < p3.x && p.x < p4.x
        || p.x > p1.x && p.x > p2.x || p.x > p3.x && p.x > p4.x
  ...
  protected void setRelationsVisible(boolean areRelationsVisible, final int relationType) {
    ...
    switch(relationType) {
      case Relation.SUPER_TYPE: relatedClassInfos = classInfo.getSuperTypes(); break;
      case Relation.SUB_TYPE: relatedClassInfos = classInfo.getSubTypes(); break;
      case Relation.COMPOSITION: relatedClassInfos = classInfo.getCompositions(); break;
      case Relation.ASSOCIATION: relatedClassInfos = classInfo.getAssociations(); break;
    }
    if(classInfo.isLoaded() && relatedClassInfos.length == 0) {
      return;
    }
    switch(relationType) {
      case Relation.SUPER_TYPE: setSuperTypesIndication(areRelationsVisible); break;
      case Relation.SUB_TYPE: setSubTypesIndication(areRelationsVisible); break;
      case Relation.COMPOSITION: setCompositionsIndication(areRelationsVisible); break;
      case Relation.ASSOCIATION: setAssociationsIndication(areRelationsVisible); break;
    }
    if(!classInfo.isLoaded()) {
    ...
    if(classPane.isClassComponentPresent(classInfo)) {
      classPane.setRelationsVisible(this, relatedClassInfos, areRelationsVisible, relationType);
    }
  }
