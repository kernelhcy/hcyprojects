...
public class ClassPane extends JPanel
{
	...
	protected static final Color SELECTION_FILL_COLOR 
								= new Color(SELECTION_COLOR.getRed(),
									SELECTION_COLOR.getGreen(), SELECTION_COLOR.getBlue(), 30);
	...
	protected static final Border TOOL_BAR_BORDER 
								= BorderFactory.createMatteBorder(0, 0, 1, 0,
											new Color(167, 166, 174));
	...
	class MouseHandler extends MouseInputAdapter
	{
		...
		public void mousePressed(MouseEvent e)
		{
			...
			originalSelectedClassComponents = (ClassComponent[]) 
									selectedClassComponentSet.toArray(new ClassComponent[0]);
		}
		...
		public void mouseDragged(MouseEvent e)
		{
			...
			{
				ClassComponent classComponent = ((ClassComponent) classComponentPane
						.getComponent(i));
	...
	public ClassPane()
	{
		...
		contentPane = new JPanel(new BorderLayout(0, 0))
		{
			public void reshape(int x, int y, int w, int h)
			{
				...
				if (cpBounds.x == x && cpBounds.y == y && cpBounds.width == w
						&& cpBounds.height == h)
				{
					return;
				}
				...
				if (bounds != null)
				{
					...
					if (w > size.width)
					{
						offsetX = Math.max(0, Math.min(-hScrollBar.getMinimum(), w - size.width)
								- Math.max(0, bounds.x + bounds.width - w
										+ (vScrollBar.isVisible() ? vScrollBar.getWidth() : 0)));
					}
					int offsetY = 0;
					if (h > size.height)
					{
						offsetY = Math.max(0, Math.min(-vScrollBar.getMinimum(), h - size.height)
								- Math.max(0, bounds.y + bounds.height - h
		...
		classComponentPane = new JPanel(null)
		{
			...
			protected void paintComponent(Graphics g)
			{
				...
				g.drawString(s, (int) Math.round(size.width - bounds.getWidth()
						- vScrollBar.getPreferredSize().width) - 3, fontMetrics.getAscent());
		...
		classComponentPane.addMouseWheelListener(new MouseWheelListener()
		{
			...
			{
				int amount = e.getScrollType() == MouseWheelEvent.WHEEL_BLOCK_SCROLL ? getScrollableBlockIncrement()
						* e.getScrollAmount()
						: e.getUnitsToScroll() * getScrollableUnitIncrement();
		...
		classComponentPane.addKeyListener(new KeyAdapter()
		{
			public void keyPressed(KeyEvent e)
			{
								...
								if (classComponent.isVisible())
								{
									...
									if (isCrawling())
									{
										SwingUtilities.invokeLater(new Runnable()
										{
											public void run()
											{
												classComponent.setRelationsVisible(true,
														Relation.SUPER_TYPE);
												classComponent.setRelationsVisible(true,
														Relation.SUB_TYPE);
												classComponent.setRelationsVisible(true,
				...
				case KeyEvent.VK_UP:
					...
					{
						moveClassComponents(0, isControlPressed ? -getScrollableBlockIncrement()
								: -getScrollableUnitIncrement());
					}
					else
					{
						moveSelectedClassComponents(0,
								isControlPressed ? -getScrollableBlockIncrement()
										: -getScrollableUnitIncrement());
					}
					break;
				...
				case KeyEvent.VK_LEFT:
					if (selectedClassComponentSet.isEmpty())
					{
						moveClassComponents(isControlPressed ? -getScrollableBlockIncrement()
								: -getScrollableUnitIncrement(), 0);
					}
					else
					{
						moveSelectedClassComponents(
								isControlPressed ? -getScrollableBlockIncrement()
										: -getScrollableUnitIncrement(), 0);
					}
					break;
		...
		hScrollBarPanel.add(new JPanel(null)
		{
			...
			{
				return vScrollBar.isVisible() ? new Dimension(vScrollBar.getWidth(), 0)
						: new Dimension(0, 0);
			}
		}, BorderLayout.EAST);
	...
	protected void setRelationsVisible(ClassComponent classComponent,
			ClassInfo[] relatedClassInfos, boolean areRelationsVisible, final int relationType)
	{
		...
		adjustClassComponents(classComponent, (ClassComponent[]) componentSet
				.toArray(new ClassComponent[0]), relationType);
	}

	protected void adjustClassComponents(ClassComponent mainClassComponent,
			ClassComponent[] classComponents, final int relationType)
	{
		if (classComponents.length == 0)
		{
			return;
		...
		for (int i = 0; i < classComponents.length; i++)
		{
			...
			Point location = (Point) classNameToLocationMap.get(classComponent.getClassInfo()
					.getClassName());
			if (location != null)
			{
				classComponent.setLocation(location);
		...
		int distance = 30
				* classComponents.length
				+ 40
				+ (int) Math.round(Math.sqrt(Math.pow(origin.x - bounds.x, 2)
						+ Math.pow(origin.y - bounds.y, 2)));
		for (int i = 0; i < classComponents.length; i++)
		{
			ClassComponent classComponent = classComponents[i];
			Rectangle cBounds = classComponent.getBounds();
	...
	protected void removeClassComponents(ClassComponent[] classComponents)
	{
		for (int i = 0; i < classComponents.length; i++)
		{
			...
			for (int j = relationList.size() - 1; j >= 0; j--)
			{
				...
				if (relation.getClassComponent1() == classComponent
						|| relation.getClassComponent2() == classComponent)
				{
	...
	{
		ClassComponent classComponent = (ClassComponent) classInfoToClassComponentMap
				.get(classInfo);
		if (classComponent == null)
		{
	...
	protected void paintClassComponentPane(Graphics g, boolean isPrinting)
	{
			...
			if (classComponent1.isVisible() && classComponent2.isVisible())
			{
				...
				if (p1 != null)
				{
					...
					if (p2 != null)
					{
						...
						case Relation.SUPER_TYPE:
						{
							int length = (int) Math.round(Math.sqrt(Math.pow((p2.x - p1.x), 2)
									+ Math.pow((p2.y - p1.y), 2)));
							int arrowLength = 12;
							if (length <= arrowLength + 2)
							{
								if (isPrinting || isClassComponentSelected(classComponent1)
										|| isClassComponentSelected(classComponent2))
								{
							...
							int x = p1.x;
							int y = p1.y;
							Polygon polygon = new Polygon(new int[] { x, x, x - arrowBase, x,
									x + arrowBase, x }, new int[] { y, y - length + arrowLength,
									y - length + arrowLength, y - length, y - length + arrowLength,
									y - length + arrowLength }, 6);
							double angle = Math.atan(((double) (p2.x - p1.x)) / (p2.y - p1.y));
							if (p1.y <= p2.y)
							{
								angle += Math.PI;
							}
							AffineTransform at = AffineTransform.getRotateInstance(-angle, p1.x,
									p1.y);
						...
						case Relation.SUB_TYPE:
						{
							int length = (int) Math.round(Math.sqrt(Math.pow((p1.x - p2.x), 2)
							...
							AffineTransform at = AffineTransform.getRotateInstance(-angle, p2.x,
									p2.y);
							g.setColor(Color.WHITE);
							((Graphics2D) g).fill(at.createTransformedShape(polygon));
							if (isPrinting || isClassComponentSelected(classComponent1)
						...
						case Relation.COMPOSITION:
						{
							...
							int y = p2.y;
							Polygon polygon = new Polygon(new int[] { x, x, x - arrowBase, x,
									x + arrowBase, x }, new int[] { y,
									y - length + diamondArrowLength * 2,
									y - length + diamondArrowLength, y - length,
									y - length + diamondArrowLength,
									y - length + diamondArrowLength * 2 }, 6);
							double angle = Math.atan(((double) (p1.x - p2.x)) / (p1.y - p2.y));
							if (p2.y <= p1.y)
							{
								angle += Math.PI;
						...
						case Relation.ASSOCIATION:
						{
							...
							int y = p1.y;
							Polygon polygon = new Polygon(new int[] { x, x, x - arrowBase, x,
									x + arrowBase, x }, new int[] { y, y - length,
									y - length + arrowLength, y - length, y - length + arrowLength,
									y - length }, 6);
							double angle = Math.atan(((double) (p2.x - p1.x)) / (p2.y - p1.y));
							if (p1.y <= p2.y)
		...
		if (classComponentPane.getComponentCount() == 0)
		{
			...
			// The getClassComponentPanePrintBounds() method should be adjusted
			// in case of a change here.
			int x = 40;
			int y = 45;
			...
			int h = fm.getHeight();
			Object aaRenderingHint = ((Graphics2D) g)
	...
	protected void moveSelectedClassComponent(ClassComponent mainClassComponent, int offsetX,
			int offsetY)
	{
	...
	public void adjustBounds()
	{
		...
		hScrollBar.setValues(0, size.width, bounds.x, Math.max(size.width, bounds.x + bounds.width
				- 1));
		vScrollBar.setValues(0, size.height, bounds.y, Math.max(size.height, bounds.y
				+ bounds.height - 1));
	}
	...
	protected void developMenu(final JComponent menuComponent, final long timeReference,
